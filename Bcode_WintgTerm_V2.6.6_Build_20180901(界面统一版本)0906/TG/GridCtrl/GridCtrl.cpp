...
{
    CPaintDC dc(this);      // device m_context for painting
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem, BOOL bModify)
{
    ...
    SetModified(bModify, pItem->row, pItem->col);
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    {
		OnSelRowCol(m_idCurrentCell.row, m_idCurrentCell.col);
	    SendMessageToParent(m_idCurrentCell.row, m_idCurrentCell.col, GVN_SELCHANGED);
...
// EFW - Added to forward right click to parent so that a m_context
