			...
			else
			{
				...
				if(m_pDoc->GetTgType() != TG_TYPE::BASIC_TG)
				{
					...
					if(m_pDoc->m_ptgctrl->mConfigTg.m_context.IsCanStopTrainAbs())
					{
						...
						{
							bool bModify=true;
							if(IsFlagTrainPartRun(sel_next_rec->flag))
							{
								bModify=false;
							}
...
void CTGForm::OnDrawAbsWorkTrain(UINT nID)
{
	...
	int uExit=-1, uEntry=-1;
	if((nFlag==0 || nFlag==1) && modifyTrain.GetRecordNum()>=2)
	{
		WORD nExitNum1=0; 
		WORD nExitList1[32]; 
		CString sExplist1[32];
		WORD entry_next1[32];
		WORD station_next1[32];
		...
		int node_index1=prec->out_station_index;
		int node_index2=last->in_station_index;
		
		do
		{
			if(node_index1>=0 && node_index1<m_pDoc->m_nDispDistanceNum)
			{
				if(m_pDoc->m_sDispDistance[node_index1].station_number!=prec->station)
				{
					break;
				}
			}
			...
			if(node_index2>=0 && node_index2<m_pDoc->m_nDispDistanceNum)
			{
				if(m_pDoc->m_sDispDistance[node_index2].station_number!=last->station)
				{
					break;
				}
			}
			...
			if(node_index1<node_index2)
				...
				nDirectFlag=UP_DIRECT;
			WORD uEntry1=0;
			m_pDoc->GetEntryExitFromNodeIndex_abs(node_index1,uEntry1,nExitList1,nExitNum1,sExplist1,entry_next1,station_next1,nDirectFlag,last->station);

			WORD nExitNum=0; 
			WORD nExitList[32]; 
			CString sExplist[32];
			WORD entry_next[32];
			WORD station_next[32];

			// 正向
			for(int i=0; i<nExitNum1; i++)
			{
				if(sExplist1[i].Find("正向行车")>=0)
				{
					nExitList[nExitNum]=nExitList1[i]; 
					sExplist[nExitNum]=sExplist1[i];
					entry_next[nExitNum]=entry_next1[i];
					station_next[nExitNum]=station_next1[i];
					nExitNum++;
				}
			}
			// 反向
			...
			for(int i=0; i<nExitNum1; i++)
			{
				if(sExplist1[i].Find("正向行车")<0 && sExplist1[i].Find("反向行车")<0)
				{
					nExitList[nExitNum]=nExitList1[i]; 
					sExplist[nExitNum]=sExplist1[i];
					entry_next[nExitNum]=entry_next1[i];
			...
			if(nExitNum >1)
			{
				for(int j=0;j<nExitNum;j++)
				{
					if(AfxMessageBox(sExplist[j], MB_YESNO) == IDYES)
					{
						uExit = nExitList[j];
						uEntry = entry_next[j];
						break;
...
{
	int nDirectFlag,nRouteSetNo;
	int node_index,nRecNum;
	...
	do 
	{
		...
		m_pDoc->GetEntryExitFromNodeIndex_abs(node_index,uEntry,nExitList1,nExitNum1,sExplist1,entry_next1,station_next1,nDirectFlag);
		...
		nRecNum = m_sFocusTrain->GetRecordNum();
		if(nRecNum == 0)
			break;
		...
		SetFlagStopAbs(temprec->flag);
		temprec->stop_abs_s_time = nOffsetTime/60 + 3;
		temprec->stop_abs_e_time = nOffsetTime/60 + 57;
		...
		{
			uEntry =  entry_next[nSel];
			nStationNo_next = station_next[nSel];
			uExit = 255;
		}		 
		...
		else if(insert_type ==  INSERT_TRAIN_GOON)
		{
			...
			if(nDirectFlag == DOWN_DIRECT)
			{
				...
				rec.out_station_index=node_index+1;
			}
			//上行车
			else
			{
		...
		if(m_sFocusTrain->AddRecord(rec, m_pDoc->GetTgType()) < 0)
		{
			...
			temp = m_sFocusTrain->GetRecordByPosition(
				m_sFocusTrain->GetRecordNum( ) - 1);

			char buffer[256];
			sprintf(buffer,"列车 %s 从车站 %s 到车站 %s 间隔时间不够",
				m_strTrainId,
				m_pDoc->GetStationName(temp->station),
				m_pDoc->GetStationName(rec.station));
...
BOOL CTGForm::newTrainSimple_ABS_B(CPoint point,int insert_type)
{
	...
	do 
	{
		...
		m_pDoc->GetEntryExitFromNodeIndex(node_index,uEntry,uExit,nDirectFlag);
	...
	if( IsTechStopTrain(*rec))
	{
		if(CTC_STATION == m_pDoc->GetStationProperty(rec->station))
		{
				...
				((rec->rec_index==m_sFocusTrain->GetRecordNum()-1) && IsFlagTerminal(rec->flag)) )
			{
				...
				{
					strText1.Format("%s, %s, 股道 %s 无站台,不能办理客运作业", 
						m_pDoc->GetTrainShowId(m_sFocusTrain),
						m_pDoc->GetStationName(rec->station), 
				...
				{
					strText2.Format("%s, %s, 动车组接入非高站台股道 %s",
						m_pDoc->GetTrainShowId(m_sFocusTrain),
						m_pDoc->GetStationName(rec->station), 
						m_pDoc->GetSideName(rec->station,  rec->arrive_side));
				}
...
void CTGForm::DrawSideNamePerFourHour(CDC* pDC, int nY, CString& sideName)
{
	...
	if(m_pctrl->mConfigTg.m_context.IsDrawSideNamePerHours())
	{
		nStep=1;
	}
...
void CTGForm::OnAbsPartRun()
{
	...
	TgTRAIN_RECORD *prec_pre = newtrain.GetRecordByPosition (m_nFocusRecordIndex - 1);
	if(prec_cur == NULL || prec_pre == NULL)
	{
		MessageBox("请选中分部运行时, 机车开出站");
		return;
	}

	CInsertSimpleDlg *dlg = new CInsertSimpleDlg("输入区间分部车次", this, 5);
	if(dlg == NULL)
	{
		m_pDoc->sysprint(0,0,"[TG]: alloc memory failed in file %s at line %d",
			__FILE__,__LINE__);
	...
	if(dlg->DoModal () == IDOK)
	{	
		prec_pre->stop_abs_s_time = 3;
		prec_pre->stop_abs_e_time = 8;

		ReSetAbsDraw(prec_pre->stop_reason);
		CancelFlagStopSignal(prec_pre->flag);
		SetFlagStopAbs(prec_pre->flag);
		...
		long midt = prec_pre->depart/2 + prec_cur->arrive/2;
		if(IsFlagStopAbs(prec_pre->flag))
			midt = prec_pre->depart + prec_pre->stop_abs_e_time*60;
		...
		CString strRetTrainId=dlg->m_sRetTrainId;
		...
		rec.rec_index = 0;
		rec.station = prec_cur->station;   //本站站号
		...
		rec.stop_abs_s_time = 20;
		rec.stop_abs_e_time = 40;

		rec.entry = prec_cur->entry;
		rec.exit = prec_cur->entry;
		rec.in_station_index=prec_cur->in_station_index;
		rec.out_station_index=prec_cur->in_station_index;
		rec.arrive_side = 0;    
		rec.depart_side = 0;
		rec.adjust_status = 0;
		rec.flag = 0;
		...
		if(prec_pre->out_station_index > prec_cur->in_station_index)
			SetFlagStopAbsDown(rec.flag);
		else
			SetFlagStopAbsUp(rec.flag); 
		rescue_train.AddRecord(rec, m_pDoc->GetTgType());
		
		rescue_train.myTrainInfo.stop_abs_time = abs(rec.arrive-midt)/60 + 20;

		rec.arrive = rec.arrive + 60*60; 
		rec.depart = rec.arrive;
		rec.arrive_train_id = strRetTrainId;
		rec.depart_train_id = strRetTrainId;
		rec.rec_index = 1;
		rec.flag = 0;
		SetFlagTerminal(rec.flag);
	...
	do 
	{
		...
		if(m_sFocusTrain != NULL)
		{
			...
			else
			{
				...
				// 设置自触标志
				bool bEnable=false;
				if(m_pDoc->m_ptgctrl->mConfigTg.m_context.IsCanSetTrigeRouteFlag() && m_pDoc->GetTgType() == TG_TYPE::WORK_TG)
				{
					if(m_pDoc->GetOperatorRight(OPER_RIGHT::TG_EDIT) || m_pDoc->GetOperatorRight(OPER_RIGHT::MARK_EDIT))
					{
						bEnable=true;
				...
				if(TERM_TG_TYPE::SHIFT_TG!=m_pctrl->GetTgTermType() && TG_TYPE::BASIC_TG!=m_pctrl->GetTGType())
				{
					if(bEnable)
						hPopupMenu->AppendMenu(MF_STRING | MF_ENABLED,	ID_ROUTE_AUTO_MARK, "设置自触标志");
					else
