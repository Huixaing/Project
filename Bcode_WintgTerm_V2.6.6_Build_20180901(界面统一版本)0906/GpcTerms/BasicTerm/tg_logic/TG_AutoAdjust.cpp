...
BOOL  CTGAutoAdjustLogic::OnlyUseFirstStopSideForNanchang(CTG_SingleTrain *ptrain, int rec_index)
{
	...
	if (TG_IsArriveRouteDoing(ori_rec) || TG_IsArriveRouteOk(ori_rec) || TG_IsArriveRouteOccupy(ori_rec))
	{
		GpcLogTrainMessage(0, 0, "[股道分配 计算]: %s(%d) 在车站%d(entry %d exit %d)该股道不可改变,接车进路正在触发", 
				ptrain->GetTrainID(),ptrain->GetTrainIndex(),ori_rec.station, ori_rec.entry, ori_rec.exit);
		return FALSE;
	}
	if (TG_IsDepartRouteDoing(ori_rec) || TG_IsDepartRouteOk(ori_rec) || TG_IsDepartRouteOccupy(ori_rec))
	{
		GpcLogTrainMessage(0, 0, "[股道分配 计算]: %s(%d) 在车站%d(entry %d exit %d)该股道不可改变,发车进路正在触发", 
				ptrain->GetTrainID(),ptrain->GetTrainIndex(),ori_rec.station, ori_rec.entry, ori_rec.exit);
		return FALSE;
	}
...
const char *CTGAutoAdjustLogic::CheckTrainManualUpdateOper(CTG_SingleTrain *pnewtrain,const CMMIOperFlag &oper_flag,DWORD& status)
{
	...
	else if((oper_flag.nFlag&3)==CMMIOperFlag::STAY_TO_PASS)
	{
		...
		if(prec)
		{
			...
			{
				sprintf(reason,"日班计划规定禁止通过,不能办理通过,你确认要通过吗?");
				status=112;
				return reason;
			}
...
void  CTGAutoAdjustLogic::CheckAndSetAlarmInfo(CTG_SingleTrain *single, long start_time, long end_time, CTG_TrainPtrGroup& group, CTG_ObjectPtrGroup& markgroup)
{
	...
	for(int rec_index=0;rec_index<single->GetTrainRecordCount();rec_index++)
	{
		...
		do
		{
			...
			if(CheckTrainPassAndStop(single,rec_index,alarm_flag,strText))
				break;
			...
			//if(CheckTrainSideInfo(single,rec_index,alarm_flag,strText))
			//	break;
...
BOOL CTGAutoAdjustLogic::CheckTrainSideInfo(CTG_SingleTrain *single, int rec_index,	unsigned long &alarm_flag,CString &strText)
{
	strText="";
	alarm_flag=0;
	CTG_TrainRecord *precord=single->GetTrainRecord(rec_index);
	...
	{
		strText.Format("%s, %s, 接入封锁的车站股道", strTrainName,strStaName);
		alarm_flag=ALARM_FLAG_SIDE_ERROR;
		return TRUE;
	}
	...
	if(STA_FAILED == pSideDef->GetSideInfo(rec->arrive_side, side))
	{
		...
		if(!bStart)
		{
			strText.Format("%s %s 没有接车股道", strTrainName, strStaName);
			alarm_flag=ALARM_FLAG_SIDE_ERROR;
			return TRUE;
		}
	}
	...
	if(STA_FAILED == pSideDef->GetSideInfo(rec->depart_side, side))
	{
		...
		if(!bTerminal)
		{
			strText.Format("%s %s 没有发车股道", strTrainName, strStaName);
			alarm_flag=ALARM_FLAG_SIDE_ERROR;
			return TRUE;
		}
	}
	...
	if(bArriveSideOK && bDepartSideOK && !bStart && !bTerminal)
	{
		if(rec->arrive_side != rec->depart_side)// 到达出发股道是否一致
		{
			...
			{
				strText.Format("客车 %s %s 到发股道不一致", strTrainName,strStaName);
				alarm_flag=ALARM_FLAG_SIDE_WARN;
				return TRUE;
			}
			else
			{
				strText.Format("列车 %s %s 到发股道不一致", strTrainName, strStaName); 
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
			}
		}
	...
	if(bStart) // 始发检查发车股道
	{
		if(bDepartSideOK)
		{
			...
			{
				strText.Format("%s %s 不能发超限车 %s", strStaName, depart_side_name, strTrainName);
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
			}
			...
			if (single->IsElectricTrain())
			{
				...
				{
					strText.Format("%s %s 无接触网,不能发电力车 %s", strStaName, depart_side_name, strTrainName);
					alarm_flag=ALARM_FLAG_SIDE_ERROR;
					return TRUE;
				}
			}
			...
			{
				strText.Format("%s %s %s 不能发此类型车", strTrainName, strStaName, depart_side_name);
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
			}
			// 检查客车
			if ( single->IsPassengerTrain())
			{
				...
				if (!TG_IsTechStopTrain(*rec))
				{
					...
					{
						strText.Format("动车组 %s %s 接入非高站台股道 %s", strTrainName, strStaName, depart_side_name);
						alarm_flag=ALARM_FLAG_SIDE_ERROR;
						return TRUE;
					}
				}
	...
	else if(bTerminal)
	{
		if(bArriveSideOK)
		{
			...
			{
				strText.Format("%s %s 不能接超限车 %s", strStaName, arrive_side_name, strTrainName);
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
			}
			...
			if (single->IsElectricTrain())
			{
				...
				{
					strText.Format("%s %s 无接触网,不能接电力车 %s", strStaName, arrive_side_name, strTrainName);
					alarm_flag=ALARM_FLAG_SIDE_ERROR;
					return TRUE;					
				}
			}
			...
			{
				strText.Format("%s %s %s 不能接此类型车", strTrainName, strStaName, arrive_side_name);
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;				
			}
			// 检查客车
			if ( single->IsPassengerTrain())
			{
				...
				if (!TG_IsTechStopTrain(*rec))
				{
					...
					{
						strText.Format("%s %s 无站台,客车 %s 不能办理客运作业", strStaName, arrive_side_name, strTrainName);
						alarm_flag=ALARM_FLAG_SIDE_ERROR;
						return TRUE;
					}
					else if (single->IsCRHTrain() && !pSideDef->isHighPlatform(rec->arrive_side) )
					{
						strText.Format("动车组 %s %s 接入非高站台股道 %s", strTrainName, strStaName, arrive_side_name);
						alarm_flag=ALARM_FLAG_SIDE_ERROR;
						return TRUE;
					}
				}
	...
	else
	{
		...
		if (bDepartSideOK && (rec->arrive_side != rec->depart_side) && STA_FAILED==pSideDef->isReceiveExceedTrain(rec->depart_side, single->GetSpaceRank(rec_index)) )
		{
			strText.Format("%s %s 不能发超限车 %s", strStaName, depart_side_name, strTrainName);
			alarm_flag=ALARM_FLAG_SIDE_ERROR;
			return TRUE;
		...
		if (single->IsElectricTrain())
		{
			...
			if (bDepartSideOK && (rec->arrive_side != rec->depart_side) && STA_FAILED==pSideDef->isCatenary(rec->depart_side) )
			{
				strText.Format("%s %s 无接触网,不能发电力车 %s", strStaName, depart_side_name, strTrainName);
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
		...
		if ( single->IsPassengerTrain())
		{
			...
			{
				strText.Format("客车 %s %s %s 客车侧线通过", strTrainName, strStaName, arrive_side_name);
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
			}
			// 客运作业
	...
	if(!bStart)
	{
		...
		{
			strText.Format("车次%s 在 %s 入口%d非法",strTrainName,strStaName, precord->m_data.entry); // stplan warn 20161019
			alarm_flag=ALARM_FLAG_ENTRY_ERROR;
			return TRUE;
		}
		else
		{
			...
			{
				strText.Format("车次%s %s 入口%d和%s 不连通",strTrainName, strStaName, rec->entry, arrive_side_name);
				alarm_flag=ALARM_FLAG_ENTRY_ERROR;
				return TRUE;
			}
		}
	...
	if(!bTerminal)
	{
		...
		{
			strText.Format("车次%s %s 出口%d非法",strTrainName,strStaName,precord->m_data.exit); // stplan warn 20161019
			alarm_flag=ALARM_FLAG_ENTRY_ERROR;
			return TRUE;
		}
		else
		{
			...
			{
				strText.Format("车次%s %s %s 和出口%d 不连通",strTrainName, strStaName, depart_side_name, rec->exit);
				alarm_flag=ALARM_FLAG_ENTRY_ERROR;
				return TRUE;
			}
		}
	...
	if(NULL != pnextrec)
	{
			...
			&& m_pBasicPara->ConvertStationEntryToAbsnum(pnextrec->GetStationNo(),pnextrec->GetEntry(),&absnum2,&dir2))
		{
			if(absnum1!=absnum2)
			{
				...
				strText.Format("车次%s无法从 %s 开到 %s",strTrainName, gpcdef.GetStationName(station1), gpcdef.GetStationName(station2)); // stplan warn 20161019
				alarm_flag=ALARM_FLAG_SIDE_ERROR;
				return TRUE;
			}
		}
...
static BOOL CheckNotToMeet(const CTG_SingleTrain *train1,const CTG_SingleTrain *train2,int recindex1,int recindex2)
{
	if(!GetTGFunctionConfig()->IsCheckTrainMeetByNoMeet())
	{
		if(train1->IsCRHTrain() && train2->GetSpaceRank()>0)
			return TRUE;
		if(train2->IsCRHTrain() && train1->GetSpaceRank()>0)
	...
	const CTG_TrainRecord *prec1=train1->GetTrainRecordNoDelete_Const(recindex1);	
	const CTG_TrainRecord *prec2=train2->GetTrainRecordNoDelete_Const(recindex2);
	if(!prec1 || !prec2)
		return FALSE;
	// 区间冲突,包括单线交会和越行
	ushort abs_num_in1=0,abs_num_in2=0,dir1,dir2,abs_num_out1=0,abs_num_out2=0;
	GetTGBasicParam()->ConvertStationEntryToAbsnum(prec1->GetStationNo(),prec1->GetEntry(),&abs_num_in1,&dir1);
	GetTGBasicParam()->ConvertStationEntryToAbsnum(prec2->GetStationNo(),prec2->GetEntry(),&abs_num_in2,&dir2);
    GetTGBasicParam()->ConvertStationEntryToAbsnum(prec1->GetStationNo(),prec1->GetExit(),&abs_num_out1,&dir1);
	GetTGBasicParam()->ConvertStationEntryToAbsnum(prec2->GetStationNo(),prec2->GetExit(),&abs_num_out2,&dir2);

	if(abs_num_in1==abs_num_in1 || abs_num_out1==abs_num_in1 || abs_num_in1==abs_num_out2 || abs_num_out1==abs_num_out2)
		{
			return TRUE;
		}
	
	// 禁会条件
	if(CheckNotToMeetRecordFlag(prec1,train2))
		return TRUE;
	if(CheckNotToMeetRecordFlag(prec2,train1))
		return TRUE;
	return FALSE;
}
...
static BOOL CheckNotToMeetWithTime(const CTG_SingleTrain *train1,const CTG_SingleTrain *train2,int recindex1,int recindex2,TIME  rec_from_depart=0,TIME  rec_to_arrival=0) //后两个参数是train1的时间,可以指定非record中的时间
{
	...
	//if(abs_num_out1==0 || abs_num_out2==0 || abs_num_in2==0)  // 乔永涛 2011-1-17, 始发abs_num_in2==0、终到时abs_num_out2==0，就不判断了
	//	return FALSE;
	int meet_type=0;
	...
	if((abs_num_out1==abs_num_in2) && (abs_num_out1!=0))  //bug
		{
	...
	if(meet_type==0 || !prela_nextrec || !prela_fromrec)
		return FALSE;
	...
	for(j=0; j<trains_in_range.GetTrainCount(); j++)
	{
		...
		if ( this_depart >= rec_arrival && rec_depart >= this_arrival )
		{
			if (GetTGBasicParam()->EnableStaSideLimitCfg())
			{
				...
				{
					strText.Format("根据limit_side.cfg,超限列车%s与%s在%s交会,请检查股道", single->GetTrainID(), pTrain->GetTrainID(), strStaName );
					alarm_flag=ALARM_FLAG_MEET_ERROR;
					break;
				}
			}
			...
			{
				strText.Format("超限列车%s和%s在%s交会,请检查股道", single->GetTrainID(), pTrain->GetTrainID(), strStaName ); // stplan warn 20161019
				alarm_flag=ALARM_FLAG_MEET_ERROR;
				break;
			}
		}
...
BOOL  CTGAutoAdjustLogic::CheckTrainStaSideInfo(CTG_SingleTrain *single,int rec_index, long start_time, long end_time, CTG_TrainPtrGroup& trains_in_range, unsigned long &alarm_flag,CString &strText)
{
	...
	for(j=0; j<trains_in_range.GetTrainCount(); j++)
	{
		...
		if ( this_depart >= rec_arrival && rec_depart >= this_arrival )
		{
			...
			{
				strText.Format("%s与%s在%s 在相同股道交会，请修改股道", single->GetTrainID(), pTrain->GetTrainID(), strStaName );
				alarm_flag=ALARM_FLAG_MEET_ERROR;
				//AfxMessageBox(strText);   // 不要在后台弹对话框，该报警在下计划报警中有
				break;
			}
		}
	}
...
BOOL  CTGAutoAdjustLogic::CheckTrain_ABSBlock(CTG_SingleTrain *single,int rec_index, unsigned long &alarm_flag,CString &strText,TIME &etime, CTG_ObjectPtrGroup& markgroup)
{
	...
	{	
		...
		strText.Format("车次 %s 通过封锁、停电、检修或者停用线路!",single->GetTrainID());
		return TRUE;
	}
	return FALSE;
}
...
BOOL  CTGAutoAdjustLogic::CheckTrainPassAndStop(CTG_SingleTrain *single,int rec_index, unsigned long &alarm_flag,CString &strText)
{
	alarm_flag=0;
	strText="";
	CTG_TrainRecord *prec=single->GetTrainRecord(rec_index);
	if(!prec)
		return FALSE;
	if(TG_IsMustNotStop(prec->m_data))  // "禁停";
	{
		if(!prec->IsPassRec())
		{
			alarm_flag=ALARM_FLAG_NOT_STOP_ERROR;
			strText.Format("车次 %s在%s不允许停车",single->GetTrainID(),gpcdef.GetStationName(prec->GetStationNo()));
		}
	}
	else if(TG_IsMustStop(prec->m_data) || prec->m_data.min_stop_time>0) // "必停,";
	{
		if(prec->IsPassRec())
		{
			alarm_flag=ALARM_FLAG_NOT_PASS_ERROR;
			strText.Format("车次 %s在%s不允许通过,营业时分 %d",single->GetTrainID(),gpcdef.GetStationName(prec->GetStationNo()),prec->m_data.min_stop_time/60);
		}
	}
	if(alarm_flag)
		return TRUE;
...
void  CTGAutoAdjustLogic::CheckTrainSideCrashForStplan(CTG_SingleTrain *single,int rec_index, long start_time, long end_time, CTG_TrainPtrGroup& group)
{
	...
	for(int j=0; j<group.GetTrainCount(); j++)
	{
		...
		if(pobj)
		{
			...
			{
				strText.Format("列车%s与%s在%s 股道冲突", single->GetTrainID(), pTrain->GetTrainID(), strStaName);
...
BOOL  CTGAutoAdjustLogic::CheckStplan(CTG_SingleTrain *single, long start_time, long end_time, CTG_TrainPtrGroup& group, CTG_ObjectPtrGroup& markgroup, int rec_index)
{
	...
	if(CheckTrainPassAndStop(single,rec_index,alarm_flag,strText))
	{
		if(alarm_flag & ALARM_FLAG_NOT_STOP_ERROR)  // "禁停";
		{
			precord->AddStplanWarnInfo(5, strText);
		}
		if(alarm_flag & ALARM_FLAG_NOT_PASS_ERROR)  // "禁通";
		{
			precord->AddStplanWarnInfo(6, strText);
		}
	}
	// 检查列车与施工在区间冲突
	TIME etime=0;
