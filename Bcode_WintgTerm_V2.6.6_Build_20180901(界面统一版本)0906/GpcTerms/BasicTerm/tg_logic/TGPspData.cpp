...
const char * GetTrainStopInfo(const CTG_TrainRecord *prec)
{
	...
	if(prec)
	{
		if(TG_IsMustNotStop(prec->m_data))
			tmpstr+="禁停";
		else if(TG_IsMustStop(prec->m_data))
			tmpstr+="必停,";
	}
...
//const char *Compare2Trains(const CTG_SingleTrain *srctrain,const CTG_SingleTrain *desttrain,DWORD &flag, int nThisSection)
//{
//	static CString tmpstr;
//	CString otherstr;
//	tmpstr.Empty();
...
//			|| prec->IsTerminalRec()!=prec_dest->IsTerminalRec())
//		{
//			tmpstr.Format("接入交出改变");  
//			break;
//		}
