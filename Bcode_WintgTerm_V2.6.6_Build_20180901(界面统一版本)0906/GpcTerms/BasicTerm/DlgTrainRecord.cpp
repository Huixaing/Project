...
BEGIN_MESSAGE_MAP(CDlgTrainRecord, CDialog)
	...
	ON_BN_CLICKED(ID_CHECK_STATION_ACK_SIDE, OnBnCheckStationSideAck)
...
void CDlgTrainRecord::InitDlgCtrlItem()
{
	...
	if(m_nFlag==1)
	{
		((CButton *)GetDlgItem(ID_CHECK_STATION_ACK_SIDE))->EnableWindow(TRUE);
		m_grid.SetEditable(FALSE);
	}
	else
	{
		((CButton *)GetDlgItem(ID_CHECK_STATION_ACK_SIDE))->EnableWindow(FALSE);
