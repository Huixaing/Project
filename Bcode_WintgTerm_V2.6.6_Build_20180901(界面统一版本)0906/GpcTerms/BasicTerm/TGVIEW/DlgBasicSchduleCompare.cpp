...
CDlgBasicSchduleCompare::CDlgBasicSchduleCompare(CWorkScheduleHolder* pHolder, CWnd* pParent) 
		: CDialog(CDlgBasicSchduleCompare::IDD,pParent)
{
	m_hViewWnd=pParent->m_hWnd;
	m_pHolder=pHolder;
}

CDlgBasicSchduleCompare::~CDlgBasicSchduleCompare()
{
}

void CDlgBasicSchduleCompare::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON_RE_COMPARE_BASIC, m_btnReCheck);
...
BEGIN_MESSAGE_MAP(CDlgBasicSchduleCompare, CDialog)
	ON_WM_CLOSE()
	ON_COMMAND(IDC_BUTTON_RE_COMPARE_BASIC,ReCompare)
END_MESSAGE_MAP()

BOOL CDlgBasicSchduleCompare::OnInitDialog()
{
...
void CDlgBasicSchduleCompare::InitGridStation()
{
	CheckStationsOfSection compare_stations;
	if(m_pHolder)
	{
		CTG_ScheduleInfo  schd_info;
		m_pHolder->GetBasicScheduleData()->GetScheduleInfo(schd_info);
	...
	for (int row = 0; row < compare_stations.count; row++)
	{
		...
		int b=compare_stations.station_list[row].setbcheck;
	
		m_staSelGridCtrl.AddRow(station,b);
	}
}

void CDlgBasicSchduleCompare::InitDlgCtrlItem()
{
...
void CDlgBasicSchduleCompare::SetCompareStation()
{
...
void CDlgBasicSchduleCompare::OnClose()
{
	this->ShowWindow(SW_HIDE);
}

void CDlgBasicSchduleCompare::OnCancel()
{
	OnClose();
}

void CDlgBasicSchduleCompare::getCompareResultXml(CString& timexml,CString& sidexml,CString& routexml)
{
	timexml="<root>";
	sidexml="<root>";
	routexml="<root>";
	...
	for(int n=0; n<station_count; n++)
	{
		...
		for(int m=0; m<m_result[n].list_trains.size(); m++)
		{
			...
			else if(a.flag==2) // 2:都有
			{
				...
				if(o.arrive!=b.arrive || o.depart!=b.depart || o.accept!=b.accept || o.giveout!=b.giveout)
				{
					timexml+="<record>";
					...
					if(o.accept==0)
						...
						str.Format("<arrive>%s</arrive>", GetTimeStr(o.arrive));
					timexml+=str;

					if(o.giveout==0)
						str.Format("<depart>终到</depart>");
					else
						str.Format("<depart>%s</depart>", GetTimeStr(o.depart));
					timexml+=str;
					...
					if(o.arrive!=b.arrive && o.depart!=b.depart)
						str.Format("<conclusion>到发点不同</conclusion>");
					else if(o.arrive!=b.arrive)
						str.Format("<conclusion>到点不同</conclusion>");
					else if(o.depart!=b.depart)
					...
					if(str1!=str2)
						str.Format("<conclusion2>预览:%s 图定:%s</conclusion2>", str1, str2);
					else
				...
				if(o.arrive_side!=b.arrive_side || o.depart_side!=b.depart_side)
				{
					...
					if(o.accept==1)
						str.Format("<dtime>%s</dtime>", GetTimeStr(o.arrive));
					else
						str.Format("<dtime>%s</dtime>", GetTimeStr(o.depart));
					sidexml+=str;
	...
	CString strText="";
	for(int i=0; i<m_basic_has.size(); i++)
	{
		CTG_SingleTrain *ptrain=m_pHolder->GetBasicScheduleData()->GetSingleTrain(m_basic_has[i]);
		if(ptrain)
		{
	...
	{
		CTG_SingleTrain *ptrain=m_pHolder->GetBasicPrevScheduleData()->GetSingleTrain(m_object_has[i]);
		if(ptrain)
		{
			strText += ptrain->GetTrainID();
			strText += "\r\n";
...
void CDlgBasicSchduleCompare::ReCompare()
{
	SetCompareStation();
	if(station_count<=0)
	{
		MessageBox("基本图比较时,发现你没有选择比较车站", "提醒");
		return;
	}
...
bool CDlgBasicSchduleCompare::IsCompareThisTrain(CTG_SingleTrain* ptrain)
{
...
void CDlgBasicSchduleCompare::generateObjInfo()
{
	if(m_pHolder==NULL)
		return;

	CTG_TrainPtrGroup groupObject;
	m_pHolder->GetBasicPrevScheduleData()->FindAllTrains(groupObject);
...
void CDlgBasicSchduleCompare::compareWithBasic()
{
	if(m_pHolder==NULL)
		return;
...
void CDlgBasicSchduleCompare::compareObjWithBasic(const ResultInfo& basicinfo, int idx)
{
...
////////////////////////////////////////////////////////////////////////////////
BOOL CStationTimeListGridForBaisc::CreateGrid(CWnd *parent,const RECT &rect,UINT ctrlid)
{
	m_train_col_count=0;
	...
	strcpy(m_train_def[m_train_col_count].caption,"预览到点");
	...
	strcpy(m_train_def[m_train_col_count].caption,"预览发点");
...
void CStationTimeListGridForBaisc::ShowTrainData(CString& xml)
{
	DeleteNonFixedRows();
	InsertRows(xml,"root/record");
}

BOOL CStationSideListGridForBaisc::CreateGrid(CWnd *parent,const RECT &rect,UINT ctrlid)
{
	m_train_col_count=0;
	strcpy(m_train_def[m_train_col_count].caption,"车站");
	sprintf(m_train_def[m_train_col_count].attr_name,"station");
	...
	strcpy(m_train_def[m_train_col_count].caption,"预览股道");
...
void CStationSideListGridForBaisc::ShowTrainData(CString& xml)
{
	DeleteNonFixedRows();
	InsertRows(xml,"root/record");
}

BOOL CStationRouteListGridForBaisc::CreateGrid(CWnd *parent,const RECT &rect,UINT ctrlid)
{
	...
	strcpy(m_result_def[m_result_col_count].caption,"预览入口");
	...
	strcpy(m_result_def[m_result_col_count].caption,"预览出口");
...
void CStationRouteListGridForBaisc::ShowTrainData(CString& xml)
{
	DeleteNonFixedRows();
	InsertRows(xml,"root/record");
}
