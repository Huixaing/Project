...
CSuperGridCtrl::CSuperGridCtrl()
{
	...
	m_psTreeLine.CreatePen(PS_SOLID, 2, RGB(166,202,249)); //192
	m_psRectangle.CreatePen(PS_SOLID, 1, RGB(0,202,0));//198
	m_psPlusMinus.CreatePen(PS_SOLID, 1, RGB(0,0,0)); //0
	m_brushErase.CreateSolidBrush(RGB(255,255,255)); //255
	m_himl = NULL;
}
	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
     ...
     //	pDC->SetTextColor (crText);
		...
		else
		{
			...
			// colors
			crBackground = GetSysColor(COLOR_WINDOW);// (COLOR_WINDOW);
    	...
    	//pDC->SetBkColor(RGB(212,232,192));
...
CImageList *CSuperGridCtrl::CreateDragImageEx(int nItem)
{
	...
	if(pDC) 
	{
		...
		if(pList->m_hImageList != NULL)
			pList->Add(&bmpMap, RGB(255,255,255));  //255
		else { 
			delete pList;
			pList=NULL;
		}
...
void CSuperGridCtrl::DrawCellBkColor(CDC *pDC, int nItem, int iSubItem)
{
  //	DrawFocusCell(pDC, lpDrawItemStruct->itemID, nColumn, nSubItemWidth);
	    // CDC *pDC = GetDC();   
		CRect rect;
