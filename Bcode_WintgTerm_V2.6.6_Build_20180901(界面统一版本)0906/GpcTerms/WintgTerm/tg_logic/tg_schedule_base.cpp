...
bool  CTG_SingleTrain::IfTrainTimeLessTime(TIME ntime) const
{
	...
	for( i=0;i<rec_count;i++)
	{
		if(i==0)
		{
			if(m_cRecordList[i].IsStartRec())
			{
				if(m_cRecordList[i].m_data.depart>ntime)
					break;
			}
			else
			{
				if(m_cRecordList[i].m_data.depart>ntime || m_cRecordList[i].m_data.arrive>ntime)
					break;
			}
		}
		if(i+1==rec_count)
		{
			if(m_cRecordList[i].IsTerminalRec())
			{
				if(m_cRecordList[i].m_data.arrive>ntime)
					break;
			}
			else
			{
...
bool  CTG_SingleTrain::IfTrainInTimeScope(TIME stime, TIME etime) const
{
	if(etime<=stime)
		return false;
	int rec_count=GetTrainRecordCount();
...
{
	CString strRet,tmp;
	strRet.Format("%d-%d:", GetTrainRecordCount(), GetTrainRecordCountWithDeleted());
...
// 比较两列车径路是否相同
bool CTG_SingleTrain::IsSamePathway(const CTG_SingleTrain& train) const
{
	if(GetTrainRecordCount()!=train.GetTrainRecordCount())
	{
		return false;
	}

	bool bSame=true;
	for(int rec_index=0; rec_index<GetTrainRecordCount(); rec_index++)
	{
		const CTG_TrainRecord *prec1=this->GetTrainRecordNoDelete_Const(rec_index);
		const CTG_TrainRecord *prec2=train.GetTrainRecordNoDelete_Const(rec_index);
		if(prec1!=NULL && prec2!=NULL)
		{
			if(prec1->GetStationNo() != prec2->GetStationNo())
			{
				bSame=false;
				break;
			}
...
BOOL   CTG_TrainScheduleBase::UpdateTrain(const CTG_ScheduleInfo &schd,const CTG_SingleTrain  &train,int source_flag)
{
	...
	{
		if(AppendTrain(schd,train,source_flag))
			return TRUE;
		else
			return FALSE;
	}
