...
BOOL  CTGFunctionConfig::InitData(CString strPath)
{
	...
	{
		LoadNewStplanWarnCfg(strPath + "\\data\\new_stplan_warning_class.xml");
	}
	else
	{
		LoadStplanWarnCfg(strPath + "\\data\\stplan_warning_class.xml");
	}
...
void  CTGFunctionConfig::LoadStplanWarnCfg(const CString pathname)
{
	...
	GpcLogSysMessage(0, "阶段计划报警配置");
	// list_warn
	int warn_id_count=0;
	int warn_id[256];
	MSXML::IXMLDOMNodeListPtr list_warn = pRootNode->selectNodes( "list_warn/warn");
	for(int m = 0; m < list_warn->Getlength(); m++)
	{
		...
		pChildNode = pNode_Warn->selectSingleNode("@stytle");
		if(pChildNode)
		{
			_variant_t v;
			v = pChildNode->GetnodeTypedValue();
		...
		warn_id[warn_id_count]=id;
		warn_id_count++;
		GpcLogSysMessage(0, "%d=%s", id, text);
	}
		
	MSXML::IXMLDOMNodeListPtr list_priority = pRootNode->selectNodes( "priority_list/priority");
	for(int p=0; p<list_priority->Getlength(); p++)
	{
		MSXML::IXMLDOMNodePtr pNode_Level = list_priority->Getitem(p);
		if(pNode_Level==NULL)
			continue;
		...
		int nColor=0;
		CString text;
		pChildNode = pNode_Level->selectSingleNode("@level");
		if(pChildNode)
		{
			_variant_t v;
			v = pChildNode->GetnodeTypedValue();
			nLevel = atoi(CString(v.bstrVal));	
			SysFreeString(v.bstrVal);
			v.Clear();
		}
		...
		if(1==nLevel || 2==nLevel || 3==nLevel || 4==nLevel)
		{
			strncpy(m_stplanLeval[nLevel].text, text.GetBuffer(), 255);
			m_stplanLeval[nLevel].clr=nColor;
			m_stplanLeval[nLevel].id_count=0;

			MSXML::IXMLDOMNodeListPtr list_ctrl = pNode_Level->selectNodes("CTRL");
			for(int p=0; p<list_ctrl->Getlength(); p++)
			{
				MSXML::IXMLDOMNodePtr pNode_Ctrl = list_ctrl->Getitem(p);
				if(pNode_Ctrl==NULL)
					continue;
		
				// @id
				pChildNode = pNode_Ctrl->selectSingleNode("@id");
				if(pChildNode)
				{
					_variant_t v;
					v = pChildNode->GetnodeTypedValue();
					...
					int count=m_stplanLeval[nLevel].id_count;
					for(int n=0; n<count; n++)
					{
						if(m_stplanLeval[nLevel].id[n]==id)
						{
							break;
						}
					}
					if(n>=count)
					{
						m_stplanLeval[nLevel].id[count]=id;
						m_stplanLeval[nLevel].id_count++;
					}
					for(int idx=0; idx<warn_id_count; idx++)
					{
						if(warn_id[idx]==id)
						{
							warn_id[idx]=0;
				...
				else
				{
					...
					// @begin_id
					pChildNode = pNode_Ctrl->selectSingleNode("@begin_id");
					if(pChildNode)
					{
						_variant_t v;
						v = pChildNode->GetnodeTypedValue();
					...
					// @end_id
					pChildNode = pNode_Ctrl->selectSingleNode("@end_id");
					if(pChildNode)
					{
						_variant_t v;
						v = pChildNode->GetnodeTypedValue();
					...
					for(int idx=begin_id; idx<=end_id; idx++)
					{
						...
						for(int n=0; n<count; n++)
						{
							if(m_stplanLeval[nLevel].id[n]==idx)
							{
								break;
							}
						}
						if(n>=count)
						{
							m_stplanLeval[nLevel].id[count]=idx;
							m_stplanLeval[nLevel].id_count++;
						}
						for(int n=0; n<warn_id_count; n++)
						{
							if(warn_id[n]==idx)
							{
								warn_id[n]=0;
	...
	// 其它报警
	// 在没有配置其它报警的情况下,不在一,二和三内的报警都属于其它报警,这样可以实现有些报警不显示但是写日志
	if(m_stplanLeval[4].id_count<=0) 
	{
		for(int idx=0; idx<warn_id_count; idx++)
		{
			if(warn_id[idx]>0)
			{
				int count=m_stplanLeval[4].id_count;
				m_stplanLeval[4].id[count]=warn_id[idx];
				m_stplanLeval[4].id_count++;
			}
		}
	}
...
void  CTGFunctionConfig::LoadNewStplanWarnCfg(const CString pathname)
{
	for(int i=0; i<sizeof(m_stplanLeval)/sizeof(m_stplanLeval[0]); i++)
	{
		m_stplanLeval[i].InitWarnIdLeval();
	...
	m_stplanLeval[1].clr=RGB(255,100,0);
	m_stplanLeval[1].id_count=0;

	strncpy(m_stplanLeval[2].text, "二级报警", 255);
	m_stplanLeval[2].clr=RGB(0,255,0);
