...
BOOL CStplanGrid::CreateGrid(CWnd *parent, const RECT &rect, UINT ctrlid)
{
	...
	m_class3.clr=cfg.clr;
	includes="";
	for(int i=0; i<cfg.id_count; i++)
	{
		m_class3.id_range.push_back(cfg.id[i]);
		if(i==0)
			tmp.Format("%d", cfg.id[i]);
		else
			tmp.Format(",%d", cfg.id[i]);
	...
	GetTGFunctionConfig()->GetWarnIdLeval(4, cfg);
	m_class.clr=cfg.clr;
	includes="";
	for(int i=0; i<cfg.id_count; i++)
	{
...
void  CStplanGrid::DisplayWarnInfo()
{
	...
	for(int i=0; i<m_class3.list.GetCount(); i++)
	{
		int row=InsertRow("");
		if(row<0)
			break;
		...
		Item.nFormat = DT_LEFT|DT_VCENTER|DT_SINGLELINE;
		Item.strText = m_class3.list.GetAt(i);
				
		SetItem(&Item);
		GetCell(row,0)->SetBackClr(m_class3.clr);
	}
...
void CStplanGrid::AddWarnInfo(int nId, CString& text)
{
	if(!m_bInitFromCfg)
	{
		GpcLogTrainMessage(0,0,"下计划时检查出不合理计划:[%d]报警:%s", nId, text);
	...
	if(m_class3.id_range.end() != std::find(m_class3.id_range.begin(), m_class3.id_range.end(), nId))
	{
		GpcLogTrainMessage(0,0,"下计划时检查出不合理计划:[%d]三级报警:%s", nId, text);
		m_class3.list.Add(text);
		return;
	}
