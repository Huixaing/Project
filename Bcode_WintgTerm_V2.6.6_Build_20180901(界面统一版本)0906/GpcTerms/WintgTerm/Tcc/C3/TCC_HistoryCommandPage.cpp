...
// 删除，需要向服务器发删除确认命令指令
void CHistoryCommandPage::OnDelPartialStockCommand()
{
	if(NULL == m_pCurCmd)
		return ;
...
// 删除，需要向服务器发删除确认命令指令
void CHistoryCommandPage::OnDelStockCommand()
{
	if(NULL == m_pCurCmd)
		return ;
...
void CHistoryCommandPage::OnContextMenu(CWnd* pWnd, CPoint point)
{
	return ;
}
...
void CHistoryCommandPage::OnTccCommandStatusChange()
{
	if(NULL == m_pCurCmd)
		return ;
