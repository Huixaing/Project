		...
		for(int m=0;m<st_basic_info[n].count;m++)
		{
			...
			if(bCondition)
			{
				...
				if(bflag==flag)
				{
					...
					{
						CString strplanside="?G";
						const char* p=GetTGBasicParam()->GetSideNameByID(station, rec->m_data.arrive_side);
						if(NULL!=p)
							strplanside=p;

						CString strbside="?G";
						p=GetTGBasicParam()->GetSideNameByID(station, st_basic_info[n].info[m].ar_side);
						if(NULL!=p)
							strbside=p;
						...
						if(gpcdef.IsFormatView())
						{
							...
							{
								CString str;
								str.Format("%s次在%s站接车股道与图定股道不一致:计划%s图定%s", trainid, gpcdef.GetStationName(station));
								ShowStatus(31, str);
							}
						}
						else
						{
							CString str;
							str.Format("%s次在%s站接车股道与图定股道不一致:计划%s图定%s", trainid, gpcdef.GetStationName(station), strplanside, strbside);
							ShowStatus(31, str);
						}
					}
					...
					{
						CString strplanside="?G";
						const char* p=GetTGBasicParam()->GetSideNameByID(station, rec->m_data.depart_side);
						if(NULL!=p)
							strplanside=p;

						CString strbside="?G";
						p=GetTGBasicParam()->GetSideNameByID(station, st_basic_info[n].info[m].de_side);
						if(NULL!=p)
							strbside=p;
						...
						if(gpcdef.IsFormatView())
						{
							...
							{
								CString str;
								str.Format("%s次在%s站发车股道与图定股道不一致:计划%s图定%s", trainid, gpcdef.GetStationName(station), strplanside, strbside);
								ShowStatus(30, str);
							}
						}
						else
