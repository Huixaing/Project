...
{   
	...
	{
		m_checkbtn.EnableWindow(TRUE);
		m_checkbtn.ShowWindow(SW_SHOW);
	}
	else
	{
...
void CLoginDlg::LoginOK()
{
	...
	sysprint(10,PMTG.ThreadId,"[LOGIN]:用户成功登陆主机! ");               				
	...
	if(GenXMLMessage_InformDCMDLogin(token,content,m_username))
	{
		...
		if(nEntityOfDCMD>0)
		{
			...
			GpcLogDebugMessage(0,"运行图交接班发送用户信息 %s.", content);
		}
	}
	CDialog::OnOK();
}
...
bool CLoginDlg::CertificateUserVersion1(char *username, char *pwd)
{
	...
	if(!dynCert.DynInit())
	{
		sysprint(10,PMTG.ThreadId,"[LOGIN]:动态口令认证服务初始化失败");
	...
	{
		sysprint(10,PMTG.ThreadId,"[LOGIN]:用户在认证服务器不存在，请联系管理员");
		AfxMessageBox("用户在认证服务器不存在，请联系管理员");				
	}    
	else if(rc == AUTH_PWD_MISMATCH)
	{
		AfxMessageBox("密码错误,请重新输入 ");
		sysprint(10,PMTG.ThreadId,"[LOGIN]:用户密码错误! ");
		m_password="";
		UpdateData(FALSE);
		m_PasswordCtrl.SetFocus();
	}
	...
	{
		sysprint(10,PMTG.ThreadId,"[LOGIN]:连接口令认证服务器出错! ");
		AfxMessageBox("连接口令认证服务器出错 ");
	}
	else if(rc == AUTH_ERR_OTHER)
	{
		sysprint(10,PMTG.ThreadId,"[LOGIN]:口令认证出错! ");
