...
void  CTgEditView::OnAddTmpTrainPassway(TmpTrainInfo* info)
{
	...
	if(pData->GetSingleTrain(trainId,sttm,endtm,FALSE,TRUE))
	{
...
void CTgEditView::SingleTrainToWork()
{
	...
	if(!pData)
		return;
	TIME sttm,endtm;
	m_schdinfo.GetViewScopeRange(sttm,endtm);
	if(pData->GetSingleTrain(m_singleTrainToWork.train_id,sttm,endtm,FALSE,TRUE))
	{
		if(MessageBox("车次已经存在，是否重复加车?","提示",MB_YESNO)!=IDYES)
			return;
	}
...
void CTgEditView::OnManualAddNewTrain()
{
	...
	CAddNewTrainDlg dlg(GetDocument(),m_schdinfo.m_nType);
	if(dlg.DoModal () != IDOK)
		return;
...
void CTgEditView::BatchManualModifySide(TRAIN_INDEX train_index)
{
	...
	DlgBatchSideModify dlg(train, strCaption, GetDocument(), this);
	if(dlg.DoModal()==IDOK)
	{
