...
DWORD CMenuButton::PopupDynamicButton(int count,struct _tagDynamicMenuInfo *menuText)
{
    // -[Feral]-----------------------------------------------------------
    // Vars and Assignment
    CMenu menu;
	...
	{
		if(menuText[i].nSelectStatus)
		{
			menu.AppendMenu(MF_STRING|MF_ENABLED,menuText[i].commandid,
				menuText[i].menuText);
		}
	}
...
void CMenuButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	pDC->DrawText(text,&rectFocus,DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    ...
    CPoint pointCenter( rect.Width() - 4, (rect.Height()/2) );
