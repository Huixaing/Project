...
BOOL CGpcdef::LoadSingleProfile(char* szFileName)
{
	...
	while (!feof(fp))	{
		...
		{ // compared to keyword 
		   ...
		   else if(!strncmp(ptr, GPCNO_FORMAT_VIEW, strlen(GPCNO_FORMAT_VIEW))) 
		   {
			   result=LoadIntValue(ptr+strlen(GPCNO_FORMAT_VIEW), m_nFomatView); 
		   }
		}
		else 
		{
