...
void CPropTreeItemEdit::DrawAttribute(CDC* pDC, const RECT& rc)
{
	...
	//modified by skybird, 2004.06.23
	...
	//	TRACE("IN EDIT RECT: (%d, %d, %d, %d) FOR %s \n", r.left, r.top, r.right, r.bottom, m_sEdit);
	}
}
...
int  CPropTreeItemEdit::GetRelaItemValue() // cuihu INT value
{
	return atoi(m_sEdit);
}

void CPropTreeItemEdit::SetRelaItemValue(int va)
{
	m_sEdit.Format(_T("%d"), va);
	return;
}
...
void CPropTreeItemEdit::OnActivate()
{
	...
	SetWindowPos(NULL, m_rc.left, m_rc.top + 4, m_rc.Width(), m_rc.Height() - 4, SWP_NOZORDER | SWP_SHOWWINDOW);
