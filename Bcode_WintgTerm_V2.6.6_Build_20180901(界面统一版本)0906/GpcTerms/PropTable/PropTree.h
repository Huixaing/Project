...
#include "PropTreeItemDateTime.h"  // CUIHU
...
typedef CArray< CPropTreeItem *, CPropTreeItem * > CTreeItemArray;
...
public:
	...
	void SortByCategory();
	void SortByAlpha();
	void SetGroupInfo(const CStringArray & groupInfo); //used to sort by category

	//just for two or one level, one root, one child, or no root
	CPropTreeItem *GetItemByTextLabel(const CString &textLabel);

	void SetPropertyValues(const CStringArray & valueArray, const CStringArray &itemName);
	void GetPropertyValues(CStringArray & valueArray, const CStringArray &itemName);
	void SetComboTitle(CString &title);

	//add skybird, 2004.08.05
	void SetPropItemReadOnly(const CStringArray &itemName, const BOOL bReadOnly[]);
	void SetAllItemsReadOnly(BOOL va);

private:
	void GetAndResetAllItems(CPropTreeItem *pItem, CTreeItemArray & itemArray);
	void SortItemByAlpha(CTreeItemArray & itemArray);
	int  FindItemByString(CTreeItemArray & itemArray, const CString & itemName);

protected:
	//add skybird cao, 2004.06.08
	CComboBox     m_comboBoxItemName;
//	CWnd          m_comboBoxContainer;
	CToolbarContainer    m_toolbarContainer;
	CTreeItemArray       m_propArray; //save prop element, when insert element, not root

	LONG	      m_itemNameHeight;
	LONG          m_toolbarHeight;

	CStringArray  m_itemGroupInfo; //used to store item group info, format [group name % item1 name % item2 name]
	...
	CString  m_szGeneralInfo;
public:
	//
	// functions used by CPropTreeItem (you normally dont need to call these directly)
	//
	void SetGeneralInfo(const char *p);
	const char *GetGeneralInfo();
