...
BOOL CTGDoc::OnNewDocument()
{
	...
	InitStationVerticalPos(true, false); 
...
void CTGDoc::ReadStationSide()
{
	...
	{	
		...
		{
			CString str;
			str.Format("在文件: %s　中找不到车站: %d %s", station_side_def_path, m_nCurrentStationList[i], m_pStationDef[i]->GetReadFileReason(rs));
			m_ptgctrl->FIRE_TG_LOG(str);
			m_pStationDef[i]->InitStationSideInfo();
		}
	}
	...
	if(bInitStaDistance)
	{
		...
		for(int idx = 0; idx < m_nDispDistanceNum; idx++)
		{
			...
			if(pNodeInfo == NULL)
			{
				...
				MessageBox(m_ptgctrl->m_hWnd, msg, "错误", MB_ICONHAND|MB_OK);
				return false;
			}
            ...
            if(pStation == NULL)
			{
				...
				MessageBox(m_ptgctrl->m_hWnd, msg, "TG错误", MB_ICONHAND|MB_OK);
				return false;
			}
			...
			LINE_INFO staline;
			this->m_ptgctrl->mConfigTg.m_lineCfg.GetStaLineCfg(nCurStationId, staline);
...
void CTGDoc::GenTrainExitEntryByStationIndex(CTG_TRAIN& train)
{
	...
	if(train.myRecord.size() == 1)
	{
		...
		else
		{
			...
			int uEntry,uExit;
			this->GetEntryExitFromNodeIndex(rec->in_station_index,uEntry,uExit, nDirect); // 单点车,首站入口
		...
		if(IsFlagGiveout(rec->flag))
		{
			...
			int uEntry,uExit;
			this->GetEntryExitFromNodeIndex(rec->out_station_index,uEntry,uExit,nDirect); // 单点车,尾站出口
			rec->exit = uExit;
		}
		else
		{
			...
			if (rec !=NULL && nxt !=NULL)
			{
				if(idx == 0)
				{
					...
					else
					{
						...
						int uEntry,uExit;
						this->GetEntryExitFromNodeIndex(rec->in_station_index,uEntry,uExit, nDirect); // 多站时,首站入口
				...
				else if(idx == train.myRecord.size()-1)
				{
					if(IsFlagGiveout(rec->flag))
					{
						...
						int uEntry,uExit;
						this->GetEntryExitFromNodeIndex(rec->out_station_index,uEntry,uExit,nDirect);// 多站时,尾站入口
						rec->exit = uExit;
					}
					else
					{
...
void CTGDoc::CheckTrainSideInfo(CTG_TRAIN *ptrain, int rec_index, CStringArray& ary)
{
	...
	if(arrive_side_name=="?G")
	{
		...
		if(!bStart)
		{
			strText.Format("%s次在%s站未分配接车股道", strTrainId, strStaName);
			ary.Add(strText);
		}
	}
	...
	if(depart_side_name=="?G")
	{
		...
		if(!bTerminal)
		{
			strText.Format("%s次在%s站未分配发车股道", strTrainId, strStaName);
			ary.Add(strText);
		}
	}
	...
	if(bArriveSideOK && bDepartSideOK && !bStart && !bTerminal)
	{
		if(arrive_side != depart_side)
		{
			if(ptrain->GetTrainType() == TYPE::PASSENGER)
				strText.Format("旅客列车%s次在%s站到发股道不一致", strTrainId, strStaName); 
			else
				strText.Format("列车%s次在%s站到发股道不一致", strTrainId, strStaName); 
			ary.Add(strText);
		}
	}
	...
	if(bStart) // 始发检查发车股道
	{
		if(bDepartSideOK)
		{
			...
			if (ptrain->myTrainInfo.space_rank>0) // 20090710 
			{
				...
				{
					strText.Format("超限列车%s次在%s站禁止由%s发车", strTrainId, strStaName, depart_side_name);
					ary.Add(strText);
				}
			}
			...
			if (ptrain->myTrainInfo.b_elec_rank)
			{
				...
				{
					strText.Format("%s次在%s站禁止由无接触网的%s发车", strTrainId, strStaName, depart_side_name);
					ary.Add(strText);
				}
			}
			...
			{
				strText.Format("%s次在%s站禁止由%s发车,违反客货列车接发条件", strTrainId, strStaName, depart_side_name);
				ary.Add(strText);
			}
			...
			if (ptrain->GetTrainType() == TYPE::PASSENGER)
			{
				...
				if(!IsTechStopTrain(*precord))
				{
					...
					{
						strText.Format("%s站%s无站台,%s次不能办理客运作业", strStaName, depart_side_name, strTrainId);
						ary.Add(strText);
					}
					else if(IsCrhTrain(ptrain->myTrainInfo) && !this->m_ptgctrl->mConfigTg.isHighPlatform(station,depart_side))
					{
						strText.Format("动车组%s次在%s站接入非高站台股道", strTrainId, strStaName);
						ary.Add(strText);
					}
				}
			}
	...
	else if(bTerminal)
	{
		if(bArriveSideOK)
		{
			...
			if (ptrain->myTrainInfo.space_rank>0) // 20090710 
			{
				...
				{
					strText.Format("超限列车%s次在%s站禁止接入%s", strTrainId, strStaName, arrive_side_name);
					ary.Add(strText);
				}
			}
			...
			if (ptrain->myTrainInfo.b_elec_rank)
			{
				...
				{
					strText.Format("%s次在%s站禁止接入无接触网的%s", strTrainId, strStaName, arrive_side_name);
					ary.Add(strText);
				}
			}
			...
			{
				strText.Format("%s次在%s站禁止接入%s,违反客货列车接发条件", strTrainId, strStaName, arrive_side_name);
				ary.Add(strText);
			}
			...
			if (ptrain->GetTrainType() == TYPE::PASSENGER)
			{
				...
				if(!IsTechStopTrain(*precord))
				{
					...
					{
						strText.Format("%s站%s无站台,%s次不能办理客运作业", strStaName, arrive_side_name, strTrainId);
						ary.Add(strText);
					}
	...
	else
	{
		...
		if (ptrain->myTrainInfo.space_rank>0) // 20090710 
		{
			...
			if (bDepartSideOK && !pStationDef->isReceiveExceedTrain(depart_side, ptrain->myTrainInfo.space_rank))
			{
				strText.Format("超限列车%s次在%s站禁止由%s发车", strTrainId, strStaName, depart_side_name);
				ary.Add(strText);
			}
		...
		if (ptrain->myTrainInfo.b_elec_rank)
		{
			...
			if (bDepartSideOK && !pStationDef->isCatenary(depart_side) )
			{
				strText.Format("%s次在%s站禁止由无接触网的%s发车", strTrainId, strStaName, depart_side_name);
				ary.Add(strText);
			}
		...
		if(bDepartSideOK && !pStationDef->isReceiveTrain(depart_side, nTrainType) )
		{
			strText.Format("%s次在%s站禁止由%s发车,违反客货列车接发条件", strTrainId, strStaName, depart_side_name);
			ary.Add(strText);
		}
		...
		if (ptrain->GetTrainType() == TYPE::PASSENGER)
		{
			...
			if(bArriveSideOK && bDepartSideOK && !bStart && !bTerminal)
			{
				if(precord->arrive == precord->depart)
				{
					...
					{
						strText.Format("%s次在%s站%s侧线通过", strTrainId,strStaName, arrive_side_name);
						ary.Add(strText);
					}
				}
			}
	...
	if(!bStart)
	{
		...
		{
			strText.Format("%s次在%s站入口线别错误",strTrainId,strStaName);
			ary.Add(strText);
		}
		else
		{
			...
			{
				strText.Format("%s次在%s站不能经由%d口接入%s",strTrainId, strStaName, precord->entry, arrive_side_name);
				ary.Add(strText);
			}
		}
	}
	...
	if(!bTerminal)
	{
		...
		{
			strText.Format("%s次在%s站出口线别错误",strTrainId,strStaName); // stplan warn 20161019
			ary.Add(strText);
		}
		else
		{
			...
			{
				strText.Format("%s次在%s站不能经由%s发往%d口",strTrainId, strStaName, depart_side_name, precord->exit);
				ary.Add(strText);
			}
		}
	}
	...
	if(NULL != pprerec)
	{
			...
			&& ConvertStationEntryToAbsnum(pprerec->station,pprerec->exit,&absnum2,&dir2))
		{
			if(absnum1!=absnum2)
			{
				strText.Format("%s次无法从%s站开到%s站",strTrainId, 
...
bool  CTGDoc::ArriveSideSetEntry(HWND hWnd, int arrive_side, int rec_index, CTG_TRAIN *pTrain)
{
	...
	else
	{
		...
		{
			CString strText;
			strText.Format("请注意:%s次在%s站不能经由%d口接入%s", 
					trainName, GetStationName(rec->station), rec->entry, GetSideName(rec->station, arrive_side));
...
bool  CTGDoc::DepartSideSetEntry(HWND hWnd, int depart_side, int rec_index, CTG_TRAIN *pTrain)
{
	...
	else
	{
		...
		{
			CString strText;
			strText.Format("请注意:%s次在%s站不能经由%s发往%d口", 
					trainName, GetStationName(rec->station), GetSideName(rec->station, depart_side), rec->exit);
			MessageBox(hWnd, strText, "错误", MB_OK|MB_ICONHAND);
...
bool CTGDoc::TrytoMoveOtherTrain(CTG_TRAIN* ptrain, CTG_TRAIN& othertrain)
{
	...
	else if(IsAbsPartRunTrain(othertrain.myTrainInfo.adjust_flag))
	{
		if(pFirst->special_train_index==ptrain->m_nTrainIndex)
		{
			...
			int count = ptrain->GetRecordNum();
			if(count>=2)
			{
				// 部分救援
				TgTRAIN_RECORD* prec=ptrain->GetRecordByRecIndex(count-2);
				TgTRAIN_RECORD* pLast=ptrain->GetRecordByRecIndex(count-1);
				if(prec!=NULL && pLast!=NULL)
				{
					if(prec->station == pFirst->station && IsFlagTrainPartRun(prec->flag) && IsFlagStopAbs(prec->flag))
					{
						int mid_time = prec->depart + prec->stop_abs_e_time*60;
					...
					else if(pLast->station == pFirst->station && IsFlagTrainPartRun(prec->flag) && IsFlagStopAbs(prec->flag))
					{
						int mid_time = prec->depart + prec->stop_abs_e_time*60;
				...
				if(pLast!=NULL)
				{
					if(pLast->station == pFirst->station && IsFlagTrainWholeHelp(pLast->flag) && IsFlagStopAbs(pLast->flag))
					{
						long mid_time = pLast->depart + pLast->stop_abs_s_time*60;
					...
					{
						WORD line_no=0;
						int station=GetAbsLineOtherStation(pLast->station, pLast->exit, line_no);
						if(station == pFirst->station && IsFlagTrainWholeHelp(pLast->flag) && IsFlagStopAbs(pLast->flag))
						{
							long mid_time = pLast->depart + pLast->stop_abs_s_time*60;
...
void CTGDoc::ReLayoutBmpPageSize(int nSelDistanceNo, HWND hWnd)
{
	const SectionDisplay* disp = this->m_ptgctrl->mConfigTg.m_tgdef.GetSectionDisplay(m_nCurrentSectionNo);
	if(disp == NULL)
	{
		CString msg;
		msg.Format("配置文件 TG.def 区段%d的显示配置找不到",m_nCurrentSectionNo);
		MessageBox(hWnd, msg, "错误", MB_ICONHAND|MB_OK);
		return;
	}
	...
	else
	{
		...
		if(pDispDistance->show_sides_mode == 0)
			pDispDistance->show_sides_mode = 1;    
		else
			pDispDistance->show_sides_mode = 0;

		if(m_ptgctrl->mConfigTg.m_context.m_bDefaultShowSide)
		{
			pDispDistance->show_sides_mode = 0;
		}
	}
...
{
	if(ptrain==NULL || m_ptgctrl==NULL)
		return;
	if(m_ptgctrl->mConfigTg.m_context.m_bModifyRelationSetTrainImport)
	{
...
int  CTGDoc::GetAbsLineOtherStation(WORD station, WORD entry_num, WORD& abs_num)
{
	WORD dir=0;
	int nOtherStation=0;
	abs_num=0;
	if(ConvertStationEntryToAbsnum(station, entry_num, &abs_num, &dir))
	{
		const ABS_LINE* pline=m_ptgctrl->mConfigTg.m_tgdef.GetLineDefine(abs_num);
		if(pline!=NULL)
		{
			if(pline->nStartStation==station)
				nOtherStation=pline->nEndStation;
			if(pline->nEndStation==station)
				nOtherStation=pline->nStartStation;
		}
	}

	return nOtherStation;
}

int  CTGDoc::GetAbsPreStationIndexByNextStationAndEntry(WORD station, WORD entry_num, bool bStopAbsUp, int in_station_index)
{
	int index=-1;
	int next_station = 0;
	WORD abs_num=0, dir=0;
	if(ConvertStationEntryToAbsnum(station, entry_num, &abs_num, &dir))
	{
		const ABS_LINE* pline=m_ptgctrl->mConfigTg.m_tgdef.GetLineDefine(abs_num);
		if(pline!=NULL)
		{
			if(pline->nStartStation==station)
				next_station=pline->nEndStation;
			if(pline->nEndStation==station)
				next_station=pline->nStartStation;
		}
	}
	...
	if(next_station != 0)
	{
		if(bStopAbsUp)
		{
			for(int i = in_station_index-1; i >= 0; i--)
			{
				if(m_sDispDistance[i].station_number == next_station)
				{
					for(int n=0; n<m_sDispDistance[i].down_line_count; n++)
					{
						if(m_sDispDistance[i].down_line_list[n]==abs_num)
						{
							index = i;
							break;
						}
		...
		{
			for(int i = in_station_index+1; i < this->m_nDispDistanceNum; i++)
			{
				if(m_sDispDistance[i].station_number == next_station)
				{
					for(int n=0; n<m_sDispDistance[i].up_line_count; n++)
					{
						if(m_sDispDistance[i].up_line_list[n]==abs_num)
						{
							index = i;
							break;
						}
...
bool CTGDoc::IsThisSectionStation(int station)
{
	bool bResult=false;
	for(int n=0; n<m_nDispDistanceNum; n++)
	{
		if(station==m_sDispDistance[n].station_number)
		{
			bResult=true;
			break;
		}
...
void AddLineNo(bool other_station_is_this_section, int abs_line, int& count, int lineno[32], int& other_count, int other_lineno[32])
{
	if(other_station_is_this_section)
	{
		if(count<32)
		{
			lineno[count] = abs_line;		
			count++;
		}
	}
	else
	{
		if(other_count<32)
		{
			other_lineno[other_count] = abs_line;
			other_count++;
		}
	}
}
	
int CTGDoc::GetEntryByNodeIndexForFrom_abs(int node_index, int nstation, int dir, int abs_line_list[32])
{
	if(node_index < 0 || node_index >= m_nDispDistanceNum)
		return 0;
	...
	int count = 0;
	int other_count = 0, other_lineno[32];
	int nomorl_count = 0, nomorl_lineno[32];
	int reverse_count = 0, reverse_lineno[32];
	if(dir == UP_DIRECT)
	{
		for(int i=0;i< m_sDispDistance[node_index].down_line_count;i++)
		{
			int abs_line = m_sDispDistance[node_index].down_line_list[i];
			const ABS_LINE* line = m_ptgctrl->mConfigTg.m_tgdef.GetLineDefine(abs_line);
			if(line)
			{
				if(station == line->nStartStation || station == line->nEndStation)
				{
					if(station == line->nStartStation)
					{
						bool bThisStation = IsThisSectionStation(line->nEndStation);
						AddLineNo(bThisStation, abs_line, reverse_count, reverse_lineno, other_count, other_lineno);
					}
					else if(station == line->nEndStation)
					{
						bool bThisStation = IsThisSectionStation(line->nStartStation);
						AddLineNo(bThisStation, abs_line, nomorl_count, nomorl_lineno, other_count, other_lineno);
					}
				}
			}
		}
	...
	for(int n=0; n<nomorl_count; n++)
	{
		abs_line_list[count] = nomorl_lineno[n];		
		count++;
		if(count>=32)
		{
			break;
	...
	for(int n=0; n<reverse_count; n++)
	{
		abs_line_list[count] = reverse_lineno[n];		
		count++;
		if(count>=32)
		{
			break;
	...
	{
		abs_line_list[count] = other_lineno[n];		
		count++;
		if(count>=32)
		{
			break;
...
int CTGDoc::GetExitByNodeIndexForTo_abs(int node_index, int nstation, int dir, int abs_line_list[32])
{
	if(node_index < 0 || node_index >= m_nDispDistanceNum)
		return 0;
...
int CTGDoc::GetExitByNodeIndex12ForTo_abs(int node_index1, int node_index2, int abs_line_list[32])
{
	if(node_index1 < 0 || node_index1 >= m_nDispDistanceNum)
		return 0;
	if(node_index2 < 0 || node_index2 >= m_nDispDistanceNum)
		return 0;
	
	if(node_index1==node_index2)
		return 0;
	
	int station=m_sDispDistance[node_index1].station_number;

	int nMix=node_index1;
	int nMax=node_index2;
	if(node_index1>node_index2)
	{
		nMix=node_index2;
		nMax=node_index1;
	...
	int count = 0, lineno[32];
	for(int i=0;i< m_sDispDistance[nMix].down_line_count;i++)
	{
		int abs_line = m_sDispDistance[nMix].down_line_list[i];
		for(int m=0;m< m_sDispDistance[nMax].up_line_count;m++)
		{
			if(abs_line == m_sDispDistance[nMax].up_line_list[m])
			{
				if(count<32)
				{
					lineno[count] = abs_line;		
		...
		if(line)
		{
			if(station == line->nStartStation || station == line->nEndStation)
			{
				if(station == line->nStartStation)
				{
					if(count<32)
					{
						nomorl_lineno[nomorl_count] = abs_line;		
						nomorl_count++;
					}
				}
				else if(station == line->nEndStation)
				{
					if(count<32)
					{
						reverse_lineno[reverse_count] = abs_line;		
						reverse_count++;
					}
				}
			}
