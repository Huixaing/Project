...
private:
    ...
    // 分部救援实现
	void CreatePartHelpRunStation(CTG_TRAIN& newtrain, CString strTrainId, CString strRetTrainId, bool bReverse);
	// 整列救援实现
	void CreateWholeHelpRunStation(CTG_TRAIN& newtrain, CString strTrainId, CString strRetTrainId, bool bReverse);
	// 根据节点获取本站出口
	int  GetThisExitByQuestion(CTG_TRAIN& train);
public:
