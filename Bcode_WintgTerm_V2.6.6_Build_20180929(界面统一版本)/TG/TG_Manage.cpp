...
void CTG_Mange::TrytoDeleteOtherTrain(CTG_TRAIN& train)
{
	...
	if(ptrain)
	{
		CTG_TRAIN newtrain=*ptrain;
		bool bSend=false;
		int n=newtrain.GetRecordNum();
		if(n>=2)
		{
			TgTRAIN_RECORD* pLast=newtrain.GetLastRecord();
			if(pLast!=NULL)
			{
				if(IsFlagTrainWholeHelp(pLast->flag))
				{
					ClearFlagTrainWholeHelp(pLast->flag);
					bSend=true;
				}
				TgTRAIN_RECORD* prec=newtrain.GetRecordByRecIndex(n-2);
				if(IsFlagTrainPartRun(prec->flag))
				{
