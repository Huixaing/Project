// AbsEleFixProperty.cpp : implementation file
//

#include "stdafx.h"
#include "IF.h"

#include "AbsEleFixProperty.h"
...
CAbsEleFixProperty::CAbsEleFixProperty(CTGDoc *pDoc,CTg_BLOCK * pFigure,CWnd* pParent /*=NULL*/)
	: CDialog(CAbsEleFixProperty::IDD, pParent)
{
...
CAbsEleFixProperty::~CAbsEleFixProperty()
{
...
void CAbsEleFixProperty::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CAbsEleFixProperty)
...
BEGIN_MESSAGE_MAP(CAbsEleFixProperty, CDialog)
	//{{AFX_MSG_MAP(CAbsEleFixProperty)
	//}}AFX_MSG_MAP
	ON_CBN_SELCHANGE(IDC_COMBO_DIRECTION, OnCbnSelchangeComboDirection)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CAbsEleFixProperty message handlers
BOOL CAbsEleFixProperty::OnInitDialog() 
{
	...
	for(int i=0; i<all_lineno.size()&&i<128; i++)
	{
		...
		if(line!=NULL)
		{
			...
			m_line_list[m_linecount].name[31]=0;
...
void CAbsEleFixProperty::CreateCheckBox()
{
	if(m_linecount<=0)
		return;
...
void CAbsEleFixProperty::OnCbnSelchangeComboDirection()
{
	int selIndex = m_direction.GetCurSel();
