	...
	: CDialog(temp, pParent)
{
	...
	if(m_pFigure!=NULL)
	{
		if(m_pFigure->type == SIDE_INHIBIT_MARK)
			m_nSubType=(m_pFigure->slow_time & 0x0001);
		else
			m_nSubType=0;
	}
}
...
void CStationInhibitProperty::InitIdentify()
{
	...
	else if(m_pFigure->type == SIDE_INHIBIT_MARK)
	{
		if(m_nSubType == 0x0001)
			this->SetWindowText("站内股道停电");
		else
