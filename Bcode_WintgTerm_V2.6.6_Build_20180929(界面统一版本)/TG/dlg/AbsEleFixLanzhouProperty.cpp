...
void CAbsEleFixLanzhouProperty::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MaxChars(pDX, m_ele_title, 255);  //实际上最大为63
...
BOOL CAbsEleFixLanzhouProperty::OnInitDialog() 
{
	...
	m_direction.AddString("上行");   // 0
	m_direction.AddString("下行");   // 1 
	m_direction.AddString("上下行"); // 2
	m_direction.AddString("单线"); // 3
	...
	{
		m_title.AddString("电网检修");
		m_title.AddString("封锁天窗");
	}
		
	if(m_pFigure->slow_time&0x0001)//低1位为1
		m_lockprestation = TRUE;
	else
		m_lockprestation = FALSE;

	if(m_pFigure->slow_time&0x0002)//低2位为1
		m_locknextstation = TRUE;
	else
		m_locknextstation = FALSE;

	if(m_pFigure->slow_time&0x0004)//低3位为1
		m_DownFlag = TRUE;
	else
		m_DownFlag = FALSE;

	if(m_pFigure->slow_time&0x0008)//低4位为1
		m_UpFlag = TRUE;
	else
		m_UpFlag = FALSE;
...
void CAbsEleFixLanzhouProperty::OnOK() 
{
	...
	//是否封锁上下方车站
	if(m_lockprestation)
		m_pFigure->slow_time |= 0x0001;//低1位置1
	else
		m_pFigure->slow_time &= ~0x0001;//低1位置0

	if(m_locknextstation)
		m_pFigure->slow_time |= 0x0002;//低2位置1
	else
		m_pFigure->slow_time &= ~0x0002;//低2位置0

	// 上下远近
	if(m_DownFlag) //  1 X X
		m_pFigure->slow_time |= 0x0004;//低3位置1
	else
		m_pFigure->slow_time &= ~0x0004;//低3位置0

	if(m_UpFlag) // 1 X X X
		m_pFigure->slow_time |= 0x0008;//低4位置1
	else
		m_pFigure->slow_time &= ~0x0008;//低4位置0
...
void CAbsEleFixLanzhouProperty::OnBnClickedRadioSjxy()//上近下远 0 1 X X
{
	m_UpFlag = FALSE;
	m_DownFlag = TRUE;
}

void CAbsEleFixLanzhouProperty::OnBnClickedRadioSjxj()//上近下近  0 0 X X
{
	m_UpFlag = FALSE;
	m_DownFlag = FALSE;
}

void CAbsEleFixLanzhouProperty::OnBnClickedRadioSyxy()//上远下远  1 1 X X
{
	m_UpFlag = TRUE;
	m_DownFlag = TRUE;
}

void CAbsEleFixLanzhouProperty::OnBnClickedRadioSyxj()//上远下近 1 0 X X
{
	m_UpFlag = TRUE;
	m_DownFlag = FALSE;
}
