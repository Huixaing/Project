...
//
	...
	CString GetTrainJiaoRelation(ULONG nTrainIndex)
	{
		CString result;
		static BYTE parms[] = VTS_UI4 ;
		InvokeHelper(0x57, DISPATCH_METHOD, VT_BSTR, (void*)&result, parms, nTrainIndex);
		return result;
	}
private:
	...
	CString CheckSideByRelation(VARIANT msg)
	{
		CString result;
		static BYTE parms[] = VTS_VARIANT ;
		InvokeHelper(0x56, DISPATCH_METHOD, VT_BSTR, (void*)&result, parms, &msg);
		return result;
	}
...
public:
	...
	CString SendTrainMsgToOCX(const CTG_SingleTrain& train, SEND_FLAG flag)
	{
		...
		do
		{
			...
			else if(flag==SEND_FLAG::Check_Relation)
				return CheckSideByRelation(byte_array);
		}while(0);
