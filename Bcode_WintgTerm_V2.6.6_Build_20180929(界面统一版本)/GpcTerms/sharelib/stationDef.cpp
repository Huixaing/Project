...
#ifdef SHUNT_SURPORT
CStationDef::CStationDef(STATION_NO station_no)
	:m_nNo(station_no)
{
	char strPath[128], strFileName[128];
	char *psDest;
	_getcwd(strPath, 128);
	if(_getcwd(strPath, 128) == NULL) return;

	psDest = strrchr(strPath, '\\');
	int nResult = psDest - strPath;
	char path1[128];
	strncpy(path1, strPath, nResult);
	path1[nResult] = 0x00;

	sprintf(strFileName, "%s%s\\%s", path1, "\\Data\\ShuntDef", "stationSide.def");//FD_SHUNT_DEF, FN_SHUNT_SIDE_DEF);
	init();
	FLoadStation(strFileName);
}

short CStationDef::AssertTleSide(SIDE_NO side_no)
{
	SIDE_T side;
	if(0 == GetSideInfo(side_no, side)) return STA_FAILED;
	return side.is_tle_side;
}
#endif
...
short CStationDef::GetSideInfo(SIDE_NAME side_name, SIDE_T& side) const
{
	...
	{
		if(!strcmp(side_name, m_side[nIndex].display_name))
		{
...
short CStationDef::GetSideInfoByTleName(SIDE_NAME side_name, SIDE_T& side) const
{
	...
	for(int nIndex=0; nIndex < m_nSideCnt; nIndex++)
	{
		if(!strncmp(side_name, m_side[nIndex].tle_side_name, sizeof(side_name)))
		{
			GetSideInfo(m_side[nIndex].no, side);
			return true;
		}
...
short CStationDef::_FLoadStationSide(char *ptr)
{
	...
	if(!IS_VALID_SIDE(no))  // 20161105 读取站细时,滤去不合法股道
	{
		CString str;
...
#ifdef SHUNT_SURPORT
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].shunt_slide_limit, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_SLIDE;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].freight_request, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_REQ;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].shunt_excessive_time, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_EXCETIME;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].light, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_LIGHT;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].water, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_WATER;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].freight_equipment, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_FREIGHT;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].prevent_slip, seps, token))
	{
		m_LoadFlag = READ_SIDE_SHUNT_PREVENT;
		return STA_FAILED;
	}
	if (STA_FAILED == GET_KEY_PROCESS(m_side[m_nCurSide].remark, seps, token))
	{
		m_side[m_nCurSide].remark[0]=0;
	}
#endif
...
short CStationDef::_FLoadStationEntry(char *ptr)
{
	...
	{
		entry.disconn_sides.push_back(side);
	}

	char* cat_tok = strtok( catenary_str, "#\0" );
	while ( cat_tok != NULL)
	{
		int nCatTok = atoi(cat_tok);
		if(nCatTok > 0)
			entry.catenary.push_back(nCatTok);
...
{
	GET_SIDE_PROPERTY_CHAR(side_no, side.display_name, side_name);
}
...
/*
short CStationDef::GetSideYard(SIDE_NO side_no, short& yard)
{
	GET_SIDE_PROPERTY_NUM(side_no, side.yard, yard);
}

short CStationDef::isYard(SIDE_NO side_no, short yard)
{
	JUDGE_SIDE_PROPERTY(side_no, side.is_tle_side, yard);
}
*/
...
#ifdef SHUNT_SURPORT
short CStationDef::GetSlideLimit(SIDE_NO side_no, short nSlideLimit) const
{
	GET_SIDE_PROPERTY_NUM(side_no, side.shunt_slide_limit, nSlideLimit);
}

short CStationDef::GetFreightRequest(SIDE_NO side_no, short& nFreightRequest) const
{
	GET_SIDE_PROPERTY_NUM(side_no, side.freight_request, nFreightRequest);
}

short CStationDef::GetShuntExcessiceTime(SIDE_NO side_no, SHUNT_TIME& nShuntExcessiveTime) const
{
	GET_SIDE_PROPERTY_NUM(side_no, side.shunt_excessive_time, nShuntExcessiveTime);
}

short CStationDef::isLight(SIDE_NO side_no) const
{
	JUDGE_SIDE_PROPERTY(side_no, side.light, 1);
}

short CStationDef::isWater(SIDE_NO side_no) const
{
	JUDGE_SIDE_PROPERTY(side_no, side.water, 1);
}

short CStationDef::GetFreightEquipment(SIDE_NO side_no, short& nEqument) const
{
	GET_SIDE_PROPERTY_NUM(side_no, side.freight_equipment, nEqument);
}

short CStationDef::isPreventSlip(SIDE_NO side_no) const
{
	JUDGE_SIDE_PROPERTY(side_no, side.prevent_slip, 1);
}

short CStationDef::GetSideRemark(SIDE_NO side_no, SIDE_REMARK& remark) const
{
	GET_SIDE_PROPERTY_CHAR(side_no, side.remark, remark);
}
...
short CStationDef::IsStationEntry(ENTRY_ID entry_id) const
{
	ENTRY_T entry;
...
short CStationDef::GetEntryLineName(ENTRY_ID entry_id, char* strName) const
{
	...
	if ( STA_SUCCESS == GetEntryInfo(entry_id, entry) )
	{
		strcpy( strName, entry.entry_name.c_str() );
		return STA_SUCCESS;
	}
	return STA_FAILED;
}
...
short CStationDef::isCatenaryEntry(ENTRY_ID entry_id) const
{
	...
	if ( STA_SUCCESS == GetEntryInfo(entry_id, entry) )
	{
		return (entry.catenary.size() > 0) ?  STA_SUCCESS : STA_FAILED;	//wsg changed. 原代码(entry.catenary > 0)无意义
	}
	return STA_FAILED;
}
...
short CStationDef::isEntrySideConnect(SIDE_NO side_no, ENTRY_ID entry_id) const
{
	...
	if ( STA_SUCCESS == GetEntryInfo(entry_id, entry) )
	{
		for (int i=0; i<entry.disconn_sides.size(); i++)
		{
