...
BOOL  CWintgNewServer::GlobalInitAppendData(const TG_MsgStamp &stamp,const CTG_SingleTrain  &train)
{
	...
	GpcLogTrainMessage(0, 0, "接收服务器批量数据 trainID %s index %u con_id %d, schedule_type %d, m_nType %d train:%s", 
		train.GetTrainID(), train.GetTrainIndex(), stamp.con_id, stamp.schedule_type, schd.m_nType, train.toString());
				...
				if(obj_type==TG_OBJ_TRAIN)
				{
					if (va==TRUE)
					{
						...
						if (obj_count!=total)
						{
							...
							CString str;
							if (schd.m_nType==CTG_ScheduleInfo::SCHEDULE_TYPE_WORK && theWintgApp.m_nFirstReqWork==0)
							{
								str.Format("调整图初始化时,运行线与服务器无法同步");
								AddToMemoryNotify(str, time(NULL)+5,1,0,0);
							}
							if (schd.m_nType==CTG_ScheduleInfo::SCHEDULE_TYPE_BASIC && theWintgApp.m_nFirstReqBasic==0)
							{
								str.Format("基本图初始化时,运行线与服务器无法同步");
								AddToMemoryNotify(str, time(NULL)+5,1,0,0);
							}
						}

						if (schd.m_nType==CTG_ScheduleInfo::SCHEDULE_TYPE_WORK&& theWintgApp.m_nFirstReqWork==0)
							theWintgApp.m_nFirstReqWork=1;
						else if (schd.m_nType==CTG_ScheduleInfo::SCHEDULE_TYPE_BASIC && theWintgApp.m_nFirstReqBasic==0)
							theWintgApp.m_nFirstReqBasic=1;
					}	
...
void CWintgNewServer::Close()
{
	...
	for(i=0;i<DATA_SOURCE_NUM;i++)
	{
		if(m_pDataStore[i])
		{
			CTG_ScheduleInfo info;
			m_pDataStore[i]->GetScheduleInfo(info); 
			if(CTgDocument::IsSendCloseWithSvr(info.m_nSectionID, info.m_nType) || theWintgApp.bWinTGExit)
			{
				GpcLogTrainMessage(0, 0, "向服务器发送Close con_id %d, type %d",
					info.m_nSectionID, info.m_nType);
				SendScheduleLoadMessage_ToServer(info,TRUE);
			}
			else
			{
				GpcLogTrainMessage(0, 0, "不能向服务器发送Close con_id %d, type %d, 存在静态连接",
					info.m_nSectionID, info.m_nType);
			}
		}
	}
