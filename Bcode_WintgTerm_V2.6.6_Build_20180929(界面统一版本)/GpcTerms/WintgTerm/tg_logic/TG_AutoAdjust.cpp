...
void CTGAutoAdjustLogic::CheckTrainManualUpdateOper(CTG_SingleTrain *pnewtrain,const CMMIOperFlag &oper_flag, CStringArray& alarmAry)
{
	alarmAry.RemoveAll();
	...
	else if((oper_flag.nFlag&3)==CMMIOperFlag::STAY_TO_PASS)
	{
		...
		if(prec)
		{
			...
			CString strTrainName   = pnewtrain->GetTrainID();
			if(TG_IsMustStop(prec->m_data))
			{
				str.Format("%s次必须在%s站停车", strTrainName, strStationName);
				alarmAry.Add(str);
			}

			if(TG_IsMustNoPass(prec->m_data))
			{
				str.Format("%s次禁止在%s站通过", strTrainName, strStationName);
				alarmAry.Add(str);
			}
			...
			{	
				...
				{
					str.Format("%s次在%s站营业%d分,不能办理通过", strTrainName, strStationName, minute);
				}
				else
				{
					str.Format("%s次在%s站营业%d分,不能办理通过", strTrainName, strStationName, minute);
			...
			{
				str.Format("%s在%s站有作业:%s",strTrainName,strStationName,pshuaigua);
				alarmAry.Add(str);
			}			
		}
	}
	else if((oper_flag.nFlag&3)==CMMIOperFlag::PASS_TO_STAY)
	{
		...
		if(prec)
		{
			...
			{
				CString str;
				str.Format("%s次禁止在%s站停车",pnewtrain->GetTrainID(),gpcdef.GetStationName(prec->GetStationNo()));
				alarmAry.Add(str);
			}
		}
	}
	else
	{
		...
		for(;i<=rec_count-1;i++)
		{
			CString err_info;		
			TIME etime=0;
			CheckTrain_ABSBlock(pnewtrain,i, err_info, etime, markgroup);
			if(err_info!="")
			{
				alarmAry.Add(err_info);
			}

			CheckTrainABSMeetInfoForStplan(pnewtrain, i, err_info, start_time, end_time, group);
			if(err_info!="")
			{
				alarmAry.Add(err_info);
			}
...
void  CTGAutoAdjustLogic::CheckAndSetAlarmInfo(CTG_SingleTrain *single, long start_time, long end_time, CTG_TrainPtrGroup& group, CTG_ObjectPtrGroup& markgroup)
{
	...
	for(int rec_index=0;rec_index<single->GetTrainRecordCount();rec_index++)
	{
		...
		do
		{
			...
			// 检查列车与施工在区间冲突
			TIME etime=0;
			CheckTrain_ABSBlock(single,rec_index,strText,etime,markgroup);
			if(strText!="")
				break;
			
			// 检查列车区间冲突
			CheckTrainABSMeetInfoForStplan(single, rec_index, strText, start_time, end_time, group);
			if(strText!="")
				break;
			
			// 检查列车站内冲突
			CheckTrainStaMeetInfo(single,rec_index, start_time, end_time,group,strText);
			if(strText!="")
				break;
...
BOOL  CTGAutoAdjustLogic::CheckTrainStaMeetInfo(CTG_SingleTrain *single,int rec_index, long start_time, long end_time, CTG_TrainPtrGroup& trains_in_range,CString &strText)
{
	strText="";		
...
//目前只检测到发股道一致的情况
BOOL  CTGAutoAdjustLogic::CheckTrainStaSideInfo(CTG_SingleTrain *single,int rec_index, long start_time, long end_time, CTG_TrainPtrGroup& trains_in_range, CString &strText)
{
	strText="";
...
////////// 施工封锁限制条件
BOOL CTGAutoAdjustLogic::CheckTrain_SideLostEle(STATION_NO  station,SIDE_ID side_id,TIME &etime, TIME &stime)
{
	TIME reg_time=etime;
	...
	for(int i=0;i<group.GetCount();i++)
	{
		...
		if(0x0001 != (mark.slow_time & 0x0001))
		{
			continue;
		}
...
BOOL  CTGAutoAdjustLogic::CheckTrain_ABSBlock(CTG_SingleTrain *single,int rec_index,CString &strText,TIME &etime, CTG_ObjectPtrGroup& markgroup)
{
	strText="";
	TIME reg_time;
...
void  CTGAutoAdjustLogic::CheckTrainABSMeetInfoForStplan(CTG_SingleTrain *single,int rec_index, CString &strText, long start_time, long end_time, CTG_TrainPtrGroup& group)
{
	strText="";
	if(rec_index<0 || rec_index>=(single->GetTrainRecordCount()-1))
		return;
...
BOOL  CTGAutoAdjustLogic::CheckStplan(CTG_SingleTrain *single, long start_time, long end_time, CTG_TrainPtrGroup& group, CTG_ObjectPtrGroup& markgroup, int rec_index)
{
	...
	// 检查列车与施工在区间冲突
	TIME etime=0;
	if(CheckTrain_ABSBlock(single,rec_index,strText,etime,markgroup))
	{
		if(!strText.IsEmpty())
		{
	...
	//检查股道冲突
	if(CheckTrainStaSideInfo(single,rec_index, start_time, end_time,group,strText))
	{
		if(!strText.IsEmpty())
		{
...
void CTGAutoAdjustLogic::CheckTrainSideInfoForStplan(CTG_SingleTrain *single, int rec_index, bool bTdcsCheck)
{
	...
	sideusedstarttm=precord->GetArriveTime(); 
	sideuseendtm = precord->GetDepartTime();
    if(CheckTrain_SideLostEle(precord->GetStationNo(),precord->m_data.arrive_side,sideusedstarttm, sideuseendtm))
	{
		strText.Format("%s次在%s站接入无电区", strTrainName,strStaName);
		precord->AddStplanWarnInfo(29, strText);
	}
	...
	else
	{
		...
		if (bDepartSideOK && STA_FAILED==pSideDef->isReceiveTrain(rec->depart_side, nTrainType) )
		{
			strText.Format("%s次在%s站禁止由%s发车,违反客货列车接发条件", strTrainName, strStaName, depart_side_name);
			precord->AddStplanWarnInfo(22, strText);
		}
	...
	if(NULL != pnextrec)
	{
			...
			&& m_pBasicPara->ConvertStationEntryToAbsnum(pnextrec->GetStationNo(),pnextrec->GetEntry(),&absnum2,&dir2))
		{
			if(absnum1!=absnum2)
			{
				...
				strText.Format("%s次无法从%s站开到%s站",strTrainName, gpcdef.GetStationName(station1), gpcdef.GetStationName(station2)); // stplan warn 20161019
	...
	while(rec_index<ptrain->GetTrainRecordCount()-1)
	{
		...
		for(int step=0;step<2;step++)
		{
			...
			if(step==CTGFunctionConfig::ADJUST_FLAGBIT_BLOCK) // 首先考虑封锁区间情况
			{
		      ...
		      CheckTrain_ABSBlock(ptrain,rec_index,strText,cau_depart,markgroup);
