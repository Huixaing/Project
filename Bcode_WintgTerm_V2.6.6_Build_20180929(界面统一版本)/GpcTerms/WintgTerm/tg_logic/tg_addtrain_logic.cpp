...
BOOL CTGAddTrainLogic::SetTrainInfoByTrainID(CTG_SingleTrain *ptrain,const TRAIN_NO train_no)
{
	...
	if(m_pBasicPara->GetTrainPrio(train_no, &type, &run_type, &prio))
	{
		...
		// 超限车bug，应在计算股道前设置超限
		int space_type = m_pConfig->GetSuperSpaceTrainType();
		if ((0xFF != space_type) && (type == space_type))
		{
			if(ptrain->GetTrainInfo()->m_data.space_rank==0)
			{
				ptrain->GetTrainInfo()->m_data.space_rank = 1;
			}
		}
	}
	return TRUE;
...
BOOL  CTGAddTrainLogic::GenTrain_OPMS(CTG_SingleTrain *ptrain,BOOL newflag) // train中是OPMS转换来的车次
{
	...
	{
		int train_type=ptrain->GetTrainType();
		if(ptrain->IsPassengerTrain()) // 客车
		{
		...
		else
		{
			...
			{
				GpcLogTrainMessage(0, 0, "列车GID:%u 车次:%s train_type:%d 配置使用TDMS股道", 
					ptrain->GetShiftTrainIndex(), ptrain->GetTrainID(), train_type);
			}
			else
			{
	...
	if(bUseOPMSShiftSchdSide)
	{
		GpcLogTrainMessage(0, 0, "列车GID:%u 车次:%s 使用TDMS股道", ptrain->GetShiftTrainIndex(), ptrain->GetTrainID());
	}
	else
	{
