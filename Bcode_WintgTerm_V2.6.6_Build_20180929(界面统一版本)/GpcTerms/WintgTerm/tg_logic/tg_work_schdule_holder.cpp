...
CString  CWorkScheduleHolder::NewTrainNoIsExist(CString trainId, CString& stfindtmrange)
{
	CString str;
	...
	TIME sttm=0,endtm=0;
	if(GetTGFunctionConfig()->IsFindTrainByViewRange())
		schdinfo.GetViewScopeRange(sttm,endtm);
	else
		schdinfo.GetShiftManagerRange(sttm,endtm);

	CTG_SingleTrain *ptrain = m_cWorkSchduleData.GetSingleTrain(trainId,sttm,endtm,FALSE,FALSE);
	if(NULL != ptrain)
	{
		...
		if(NULL != prec)
		{
			int ntm=0;
			if(prec->IsStartRec())
				ntm = prec->GetDepartTime();
			else
				ntm = prec->GetArriveTime();
			CString strTime;
			if(ntm>0)
			{
				CTime t(ntm);
				strTime=t.Format("%d日-%H:%M");
			}
			str.Format("%s(%s %s)", trainId, gpcdef.GetStationName(prec->GetStationNo()), strTime);

			if(sttm>=0 && endtm>=0 && stfindtmrange=="")
			{
				CTime t1(sttm), t2(endtm);
				stfindtmrange.Format("车次查找范围:%s--%s", t1.Format("%d日-%H:%M"), t2.Format("%d日-%H:%M"));
			}
		}
	}
	return str;
...
void CWorkScheduleHolder::CheckTrainManualUpdateOper(CTG_SingleTrain *pnewtrain,const CTG_ScheduleInfo &schd,const CMMIOperFlag &oper_flag, CStringArray& alarmAry)
{
	alarmAry.RemoveAll();
	if(schd.m_nType!=CTG_ScheduleInfo::SCHEDULE_TYPE_WORK)
		return;
	...
	adjust_logic.CheckTrainManualUpdateOper(pnewtrain,oper_flag,alarmAry);
}
...
CString CWorkScheduleHolder::Get_BasicTrainId(TRAIN_INDEX train_index)
{
	CString strTrainId="";
	CTG_SingleTrain *ptrain=m_cBasicSchduleData.GetSingleTrain(train_index);
	if(NULL != ptrain)
	{
		strTrainId=ptrain->GetTrainID();
	}
	return strTrainId;
}
	
CString CWorkScheduleHolder::Get_ShiftTrainId(TRAIN_INDEX train_index)
{
	CString strTrainId="";
	CTG_SingleTrain *ptrain=m_cShiftSchduleData.GetSingleTrain(train_index);
	if(NULL != ptrain)
	{
		strTrainId=ptrain->GetTrainID();
	}
