...
///////////////逻辑检查
	...
	void  CheckTrainABSMeetInfoForStplan(CTG_SingleTrain *single, int rec_index, CString &strText, long start_time, long end_time, CTG_TrainPtrGroup& group);
	...
	BOOL  CheckTrainStaMeetInfo(CTG_SingleTrain *single,int rec_index, long start_time, long end_time, CTG_TrainPtrGroup&  trains_in_range, CString &strText);
	//added by zhaoliang for side check
	BOOL  CheckTrainStaSideInfo(CTG_SingleTrain *single,int rec_index, long start_time, long end_time, CTG_TrainPtrGroup&  trains_in_range, CString &strText);
	
	BOOL  CheckTrain_ABSBlock(CTG_SingleTrain *single,int rec_index,CString &strText,TIME &etime, CTG_ObjectPtrGroup& markgroup);
	
	void  CheckTrainManualUpdateOper(CTG_SingleTrain *pnewtrain,const CMMIOperFlag &oper_flag, CStringArray& alarmAry);
	
	void  SetTrainPassNonMainSide(CTG_SingleTrain *ptrain);

	
	BOOL  CheckTrain_SideLostEle(STATION_NO  station,SIDE_ID side_id,TIME &etime, TIME &stime);
	BOOL  CheckTrain_SideBlock(STATION_NO  station,SIDE_ID side_id,TIME &etime, TIME &stime);
	void  CheckTrainPassAndStop(CTG_SingleTrain *single, int rec_index);
