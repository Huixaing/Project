...
void  CSideGridView::DisplaySideInfo(const char *xml,const char *rootnode)
{
	...
	for(int i = 0; i <rec_count; i++)
	{
		...
		int ctrl=0;
		pNode = pRecAbsList->Getitem(i)->selectSingleNode("CTRL");
		if(pNode)
		{
			_variant_t v;
			v = pNode->GetnodeTypedValue();
			ctrl = atoi(CString((v.bstrVal)));
			SysFreeString(v.bstrVal);
			v.Clear();
		}
		if(ctrl==1)
		{
			this->SetModified(TRUE, row, 2);
			GetCell(row,2)->SetData(1);
			SetItemBkColour(row,0,RGB(192,192,0));
		}
		else
		{
			this->SetModified(FALSE, row, 2);
			GetCell(row,2)->SetData(0);
			SetItemBkColour(row,0,RGB(192,192,0));
			SetItemBkColour(row,1,RGB(192,192,192));
			SetItemBkColour(row,2,RGB(192,192,192));
		}
...
void  CSideGridView::BeginEditCell(int row,int col)
{
	if(m_FiledInfo[col].flag==30) // 股道
	{
		...
		int ctrl=GetCell(row,2)->GetData();
		if(ctrl==0)
		{
...
DlgBatchSideModify::DlgBatchSideModify(const CTG_SingleTrain& train, CString strCaption, CTG_TrainScheduleBase *pBasicData, CTG_TrainScheduleBase *pData, CWnd* pParent /*=NULL*/)
...
bool DlgBatchSideModify::IsThisStation(int station)
{
	if(m_thisStation.size()==0)
		return true;
	if(std::find(m_thisStation.begin(), m_thisStation.end(), station)
		!= m_thisStation.end())
	{
		return true;
	}
...
void DlgBatchSideModify::SetThisStation(std::vector<int>& thisStation)
{
	m_thisStation.clear();
	m_thisStation.assign(thisStation.begin(), thisStation.end());
}
...
void DlgBatchSideModify::DisplayTrainSide()
{
	...
	for(int n=0; n<m_train.GetTrainRecordCount();n++)
	{
		...
		xml+="<RECORD>";
		CString tmp;
		if(IsThisStation(station))
			tmp.Format("<CTRL>1</CTRL>");
		else
			tmp.Format("<CTRL>0</CTRL>");
		xml += tmp;
...
{
	// TODO: 在此添加控件通知处理程序代码
	CTGAutoAdjustLogic adjust_logic(GetTGFunctionConfig(),GetTGBasicParam(), m_pBasicData, m_pData);

	GpcLogTrainMessage(0, 0, "调度员点击确定");
	CStringArray ary_list;
	for(int rec_index=0; rec_index<m_train.GetTrainRecordCount(); rec_index++)
	{
		CTG_TrainRecord *precord = m_train.GetTrainRecordByRecIndex(rec_index);
		if(precord)
		{
			precord->ClearStplanWarnInfo();
			adjust_logic.CheckTrainSideInfoForStplan(&m_train, rec_index, true);
