...
void CSelectScheduleTrain::OnSbtAck() 
{
	...
	if(bCheck && GetTGFunctionConfig()->IsBatchAddTrainFindExistTrain())
	{
		CString strTitle;
		...
		for(int i=0;i < m_nCount;i++)
		{
			if(info[intArray[i]].exist_train == CLBTH::MsgBasicTrainListReply::BT_EXIST)
				continue;
			CString str = m_pHolder->NewTrainNoIsExist(info[intArray[i]].train_id, strTitle);
			if(str != "")
			{
				ary_list.Add(str);
			}
		...
		if(ary_list.GetCount()>0)
		{
			...
			str += "以上车次已经存在, 你仍然确认加车吗?";

			if(MessageBox(str, strTitle, MB_YESNO)!=IDYES)
			{
				return;
			}
		}
