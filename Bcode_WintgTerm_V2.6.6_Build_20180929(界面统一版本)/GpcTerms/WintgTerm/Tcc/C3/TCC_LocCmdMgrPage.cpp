...
void CLocalCommandPage::AppendCommand(RAW_STOCK_COMMAND& cmd)
{
	...
	item.strText = str;
	//item.lParam = (DWORD)cmd.nKmMark[1].dw;
	item.nFormat = grid.GetItemFormat(0, item.col);
	grid.SetItem(&item);
...
BEGIN_MESSAGE_MAP(CLocalCommandPage, CPropertyPage)
	ON_BN_CLICKED(IDC_TCC_DELCMD_BTN, OnDelRawStockCommand)
	ON_NOTIFY(NM_CLICK, GRIDCTRL, OnNMClickTccCmdList)
	ON_NOTIFY(NM_RCLICK, GRIDCTRL, OnNMClickTccCmdList)
	ON_WM_CONTEXTMENU()
	ON_WM_DESTROY()
	ON_COMMAND(ID_DEL_RAWSTOCKCOMMAND, OnDelRawStockCommand)
	ON_WM_ACTIVATE()
...
// 删除 限速命令(从本地删除)
void CLocalCommandPage::OnDelRawStockCommand()
{
	if(NULL == m_pCurCmd)
		return ;
