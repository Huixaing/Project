...
BOOL Serialize(MSXML2::IXMLDOMNodePtr pCmdNode, RAW_STOCK_COMMAND &cmd, BOOL bLoad)
{
	...
	do {
		...
		// 调度命令号
		if(pNode = pCmdNode->selectSingleNode("@dcmd_id"))
		{
			if(bLoad)
			{
				varVal = pNode->GetnodeTypedValue();
				str = varVal.bstrVal;
				cmd.dcmd_id = str.GetString();
			}
			else
			{
				str.Format("%s", (const char*)cmd.dcmd_id);
				varVal = str;
				pNode->PutnodeTypedValue(varVal);
			}
			SysFreeString(varVal.bstrVal);
		...
		if(pNode = pCmdNode->selectSingleNode("@BeginKmMark"))
		{
			...
			{
				str.Format("%lu", (DWORD)cmd.nKmMark[0].GetDWORD());
				varVal = str;
				pNode->PutnodeTypedValue(varVal);
			}
			SysFreeString(varVal.bstrVal);
		...
		// BeginKmMarkSys
		if(pNode = pCmdNode->selectSingleNode("@BeginKmMarkSys"))
		{
			if(bLoad)
			{
				varVal = pNode->GetnodeTypedValue();
				str = varVal.bstrVal;
				cmd.nKmMark[0].flag = atoi(str);
			}
			else
			{
				str.Format("%lu", (DWORD)cmd.nKmMark[0].flag);
				varVal = str;
				pNode->PutnodeTypedValue(varVal);
			}
			SysFreeString(varVal.bstrVal);
		...
		if(pNode = pCmdNode->selectSingleNode("@EndKmMark"))
		{
			...
			{
				str.Format("%lu", (DWORD)cmd.nKmMark[1].GetDWORD());
				varVal = str;
				pNode->PutnodeTypedValue(varVal);
			}
			SysFreeString(varVal.bstrVal);
		...
		// EndKmMarkSys
		if(pNode = pCmdNode->selectSingleNode("@EndKmMarkSys"))
		{
			if(bLoad)
			{
				varVal = pNode->GetnodeTypedValue();
				str = varVal.bstrVal;
				cmd.nKmMark[1].flag = atoi(str);
			}
			else
			{
				str.Format("%lu", (DWORD)cmd.nKmMark[1].flag);
				varVal = str;
				pNode->PutnodeTypedValue(varVal);
			}
			SysFreeString(varVal.bstrVal);
		...
		for(int i=0; i<pNodeList->Getlength(); i++)
		{
			...
			// 受令站 开始公里标
			if(pNode = pNode_TCC->selectSingleNode("@km_begin"))
			{
				if(bLoad)
				{
					varVal = pNode->GetnodeTypedValue();
					str = varVal.bstrVal;
					cmd.tcc[i].nKmMarkFlag[0] = atoi(str);
				}
				else
				{
					str.Format("%lu", (DWORD)cmd.tcc[i].nKmMarkFlag[0]);
					varVal = str;
					pNode->PutnodeTypedValue(varVal);
				}
				SysFreeString(varVal.bstrVal);
			...
			// 受令站 结束公里标
			if(pNode = pNode_TCC->selectSingleNode("@km_end"))
			{
				if(bLoad)
				{
					varVal = pNode->GetnodeTypedValue();
					str = varVal.bstrVal;
					cmd.tcc[i].nKmMarkFlag[1] = atoi(str);
				}
				else
				{
					str.Format("%lu", (DWORD)cmd.tcc[i].nKmMarkFlag[1]);
					varVal = str;
					pNode->PutnodeTypedValue(varVal);
				}
				SysFreeString(varVal.bstrVal);
		...
		for(i=0; i<pNodeList->Getlength(); i++)
		{
			...
			if(pNode = pNode_TRAIN->selectSingleNode("@speed2"))
			{
				if(bLoad)
				{
					varVal = pNode->GetnodeTypedValue();
					str = varVal.bstrVal;
					cmd.train_cmd[i].nSpeed = atoi(str);
				}
				else
				{
					str.Format("%lu", cmd.train_cmd[i].nSpeed);
					varVal = str;
					pNode->PutnodeTypedValue(varVal);
				}
				SysFreeString(varVal.bstrVal);
			...
			{
				if(bLoad)
					cmd.train_cmd[i].nSpeed = cmd.train_cmd[i].nSpeedMaxVal;
			}
...
BOOL SaveCommand(RAW_STOCK_COMMAND& cmd, LPCSTR szFilename)
{
	...
	do {
		...
		str.Format("\n\t<COMMAND id=\"%d\" raw_sysid=\"%s\" dcmd_id=\"%s\" console=\"%d\" isCancelLimit=\"%d\" "
			"BeginStation=\"%d\" EndStation=\"%d\" AbslineID=\"%d\" "
			"isFullLimit=\"%d\" "
			"BeginKmMark=\"%d\" BeginKmMarkSys=\"%d\" EndKmMark=\"%d\" EndKmMarkSys=\"%d\" "
			"isStartNow=\"%d\" BeginTime=\"%lu\" isEndless=\"%d\" EndTime=\"%lu\" "
			"creator=\"%d\" checkor=\"%d\" confirmor=\"%d\" bGenCanCelCmd=\"%d\" >", 
			cmd.nCmdID, SysidToString(cmd.raw_sysid), (const char*)cmd.dcmd_id, cmd.nConsoleID, cmd.bCancelLimit, 
			cmd.nStaID[0], cmd.nStaID[1], cmd.nLineID,
			cmd.bFullLimit, 
			cmd.nKmMark[0].GetDWORD(), cmd.nKmMark[0].flag, cmd.nKmMark[1].GetDWORD(), cmd.nKmMark[1].flag,
		...
		{
			str.Format("\n\t\t\t<TCC id=\"%lu\" line=\"%lu\" checkor=\"%lu\" km_begin=\"%lu\" km_end=\"%lu\" />",
				cmd.tcc[j].nID,
				cmd.tcc[j].nTccLineID,
				cmd.tcc[j].nCheckorID,
				cmd.tcc[j].nKmMarkFlag[0],
				cmd.tcc[j].nKmMarkFlag[1]);
			file.WriteString(str);
		}
		...
		for(int i=0; i<sizeof(cmd.train_cmd)/sizeof(cmd.train_cmd[0]); i++)
		{
			...
			str.Format("\n\t\t<TRAINCMD sys_id=\"%s\" speed=\"%d\" speed2=\"%d\" TrainID=\"%s\">",
