// Tcc\StockCommandInputDlg.cpp : 实现文件
//
...
IMPLEMENT_DYNAMIC(CStockCommandInputDlg, CDialog)
CStockCommandInputDlg::CStockCommandInputDlg(
	RAW_STOCK_COMMAND* pcmd,
	BOOL bCheckCommand,
	ITccClientInfoProvider *pInfoProvider, 
	ITccAdviseSink *pAdviseSink,
	CWnd* pParent /*=NULL*/)
	: CDialog(CStockCommandInputDlg::IDD, pParent)
...
void CStockCommandInputDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CStockCommandInputDlg, CDialog)
	ON_WM_DESTROY()
...
// CStockCommandInputDlg 消息处理程序
BOOL CStockCommandInputDlg::OnInitDialog()
{
...
void CStockCommandInputDlg::OnDestroy()
{
	CDialog::OnDestroy();
...
//选择命令类型
void CStockCommandInputDlg::OnCbnSelchangeTccCmdType()
{
	m_nStaBegin = 0;
	m_nStaEnd   = 0;
	m_nLineID   = 0;
...
// 区间限速和区间全限速：结束站列表变化
void CStockCommandInputDlg::OnCbnSelchangeTccCmdSta()
{
	m_nStaEnd   = 0;
	m_nLineID   = 0;
	m_nMinKm    = 0;
...
void CStockCommandInputDlg::OnCbnSelchangeTccCmdSta2()
{
	m_nLineID   = 0;
	m_nMinKm    = 0;
	m_nMaxKm    = 0;
...
void CStockCommandInputDlg::OnCbnSelchangeTccLine()
{
	UpdateData(TRUE);
	m_nMinKm    = 0;
	m_nMaxKm    = 0;
...
void CStockCommandInputDlg::OnSelDirection()
{
	for(int i=0; i<1; i++)
	{
...
void CStockCommandInputDlg::OnKmMarkerChanged()
{
	if( !UpdateData(TRUE) )
		return ;
...
void CStockCommandInputDlg::AdjustKmMarker()
{
	CString str;
	m_nKmBeginAJUST=-1;
	m_nKmEndAJUST=-1;
...
void CStockCommandInputDlg::OnBnClickedBtnAdjustAck() //IDC_BTN_ADJUST_ACK
{
...
void CStockCommandInputDlg::OnBtnClickedTccCmdTimeendRadio2()
{
	m_tmEnd.EnableWindow(FALSE);
}

void CStockCommandInputDlg::OnBtnClickedTccCmdTimeendRadio()
{
	m_tmEnd.EnableWindow(TRUE);
}

void CStockCommandInputDlg::OnBtnClickedTccCmdTimebeginRadio2()
{
	m_tmBegin.EnableWindow(FALSE);
}

void CStockCommandInputDlg::OnBtnClickedTccCmdTimebeginRadio()
{
	m_tmBegin.EnableWindow(TRUE);
}

void CStockCommandInputDlg::OnSketchMap()
{
	UpdateData(TRUE);
	dir_t dir=DIR_UNKNOWN;
	switch (m_nDirection)
...
void CStockCommandInputDlg::OnOK()
{
	...
	do {
		...
		if(!m_pInfoProvider->IsValidCommandID(cmd.nCmdID))
		{
...
void CStockCommandInputDlg::OnCancel()
{
	// TODO: 在此添加专用代码和/或调用基类
...
void  CStockCommandInputDlg::OnChangeTccCmdID()
{
	if(!UpdateData())
		return;
...
void CStockCommandInputDlg::OnBtnClickedAutoComputeCmdID()
{
   m_nCmdID = m_pInfoProvider->GetCommandID();
	CString str;
...
void CStockCommandInputDlg::RequestCommandID(cmdid_t nCmdID)
{
...
LRESULT CStockCommandInputDlg::OnServerConfirmCommandID(WPARAM wParam, LPARAM lParam)
{
	// 服务器已经确认过
	if(m_bCmdIDConfirmed)
		return FALSE;
...
CStockCommandInputDlg::CTccList::CTccList()
{
}
CStockCommandInputDlg::CTccList::~CTccList()
{
...
BOOL CStockCommandInputDlg::CTccList::Create(const RECT& rect, CWnd *pParent, UINT nID)
{
	...
	colinfo[] =
	{
		{"受令站", 200, DT_LEFT | DT_WORDBREAK|DT_NOPREFIX },
...
void CStockCommandInputDlg::CTccList::Clear()
{
	m_tccs.clear();
	grid.SetRowCount(grid.GetFixedRowCount());
}
void CStockCommandInputDlg::CTccList::SetTccList(vector<RAW_STOCK_COMMAND::_TCC_STRUCT>& tccs, ITccClientInfoProvider *pInfoProvider)
{
	Clear();
...
void  CStockCommandInputDlg::CTccList::GetTccList(vector<RAW_STOCK_COMMAND::_TCC_STRUCT>& tccs)
{
	tccs = m_tccs;
}
