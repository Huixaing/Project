...
CString CTgDocument::GetPhasePlanFileDir()
{
	CString filepath;
	filepath.Format("%s\\phase_plan%d", 
		gpcdef.GetXMLDir(), m_nCurrentSectionNo);
	return filepath;
}

void CTgDocument::SavePhasePlanToFile(const CPlanInformListItem* item)
{
	if(NULL==item)
		return;
	// 1. 先删除
	CString filepath;
	filepath.Format("%s\\PLAN-%d-*.xml", 
		GetPhasePlanFileDir(), item->m_id);
	...
	if(find.FindFile(filepath))
	{
		...
		while(ffrc)
		{
			...
			{
				GpcLogTrainMessage(0,0,"删除调整计划成功 %s", filePath);
			}
			else
			{
				GpcLogTrainMessage(0,0,"删除调整计划失败 %s", filePath);
			}
		}
	}

	CString filename;
	filename.Format("%s\\PLAN-%d-%d-%d-%s.xml", 
		GetPhasePlanFileDir(), 
		item->m_id,
		item->m_recvtime, 
		item->m_signtime, 
	...
	if(!WriteGenernalXMLFile(item->m_content, filename))
	{
		GpcLogTrainMessage(0,0,"保存调整计划失败 %s", filename);
	}
	else
	{
		GpcLogTrainMessage(0,0,"保存调整计划成功 %s", filename);
	}
}

BOOL CTgDocument::LoadPhasePlanFromFile()
{
	m_listPlanInform.clear();

	CString filepath;
	filepath.Format("%s\\PLAN-*.xml", GetPhasePlanFileDir());
	...
	if(find.FindFile(filepath))
	{
		...
		while(ffrc)
		{
			...
			CString filePath=find.GetFilePath();
			CTime fctm;
			find.GetCreationTime(fctm);
			if(fctm.GetTime()+10*3600<ctm.GetTime())
			{
				DeleteFile(filePath);
			}
		}
	...
	if(find.FindFile(filepath))
	{
		...
		while(ffrc)
		{
			...
			if(ReadGenernalXMLFile(xml,filePath))
			{
				...
				CString tmp = fileTitle, str;
				int i=tmp.Find('-');
				if(i<=0)
				{
					continue;
				}
				tmp = tmp.Right(tmp.GetLength()-i-1);
				i=tmp.Find('-');
				if(i<=0)
				{
					continue;
				...
				str = tmp.Left(i);
				item.m_signtime = atol(str);
				item.m_token = tmp.Right(tmp.GetLength()-i-1);
