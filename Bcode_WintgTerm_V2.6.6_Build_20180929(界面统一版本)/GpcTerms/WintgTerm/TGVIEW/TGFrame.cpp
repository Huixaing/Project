...
LRESULT CTgFrame::OnUserSelectFrame(WPARAM wParam, LPARAM lParam)
{
    ...
    if(gpcdef.GetUserIdentify()==GPC_SCHEDULE)
	{
		...
		if(pv)
		{
			m_gpcContext.mSectionId=pv->GetViewSectionNo();
			CTG_ScheduleInfo::SCHEDULE_TYPE schd_type=pv->GetViewScheduleType();
			if(schd_type==CTG_ScheduleInfo::SCHEDULE_TYPE_WORK)
			{
				m_gpcContext.mShiftID=pv->GetViewShiftID();
			}
		}
	}
	return 0;
