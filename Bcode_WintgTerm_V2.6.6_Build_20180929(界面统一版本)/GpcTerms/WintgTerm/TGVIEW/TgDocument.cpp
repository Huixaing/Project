...
void CTgDocument::SetTrainReverseFlag(CTG_SingleTrain *ptrain)
{
	...
	for (int i=0; i<rec_count; i++) 
	{
		...
		//else if ((i==rec_count-1) && ((i-1)>=0))
		//{
		//	if(rec[i].m_data.station==rec[i-1].m_data.station && TG_IsFlagStopAbs(rec[i-1].m_data.flag))
		//  {
		//		BOOL bIsDownAbs = GetTGBasicParam()->IsAbsDirectionDown(rec[i].m_data, TRAIN_INTO_STATION);
		//		BOOL bIsUpTrain = GetTGBasicParam()->IsTrainDirectionUp(rec[i].m_data, TRAIN_INTO_STATION);

		//		if(bIsDownAbs && bIsUpTrain)
		//		{
		//			TG_SetFlagReverse(rec[i].m_data.flag);
		//			GpcLogTrainMessage(0, 0, "%s 在车站%d反向行车", ptrain->GetTrainID(), rec[i].m_data.station);
		//			GpcLogTrainMessage(0, 0, "区间方向: %s", GetTGBasicParam()->GetAbsDirectInfo(rec[i].m_data, TRAIN_INTO_STATION));
		//			GpcLogTrainMessage(0, 0, "列车方向: %s", GetTGBasicParam()->GetTrainDirectInfo(rec[i].m_data, TRAIN_INTO_STATION));

		//		}
		//		else
		//		{
		//			BOOL bIsUpAbs     = GetTGBasicParam()->IsAbsDirectionUp(rec[i].m_data, TRAIN_INTO_STATION);
		//			BOOL bIsDownTrain = GetTGBasicParam()->IsTrainDirectionDown(rec[i].m_data, TRAIN_INTO_STATION);
		//			if(bIsUpAbs && bIsDownTrain)
		//			{
		//				TG_SetFlagReverse(rec[i].m_data.flag);
		//				GpcLogTrainMessage(0, 0, "%s 在车站%d反向行车", ptrain->GetTrainID(), rec[i].m_data.station);
...
void CTgDocument::SetTrainSpaceGaurdMilitaryRank(CTG_SingleTrain& train)
{
	// 扩大超限范围原则:本站到达超限, 上站出发也超限;本站出发超限 下站到达也超限; 
	for(int a=0; a<train.GetTrainRecordCount(); a++)
	{
		CTG_TrainRecord *prec=train.GetTrainRecordByRecIndex(a);
		if(prec)
		{
			//本站到达超限
			if(prec->GetArriveSpace()>0 && a>0)
			{
				CTG_TrainRecord *pprec=train.GetTrainRecordByRecIndex(a-1);
				if(pprec)
				{
					// 上站出发也超限
					pprec->SetDepartSpace(1);
				}
			}
			
			// 本站出发超限
			if(prec->GetDepartSpace()>0 && a<train.GetTrainRecordCount()-1)
			{
				CTG_TrainRecord *pnext=train.GetTrainRecordByRecIndex(a+1);
				if(pnext)
				{
					// 下站到达也超限
					pnext->SetArriveSpace(1);
				}
			}
		}
	}
	...
	for(int a=0; a<train.GetTrainRecordCount(); a++)
	{
		...
		if(prec)
		{
			BYTE nArriveRank = prec->GetArriveSpace();
			BYTE nDepartRank = prec->GetDepartSpace();
			if(nArriveRank != 0 || nDepartRank != 0)
			{
				train.GetTrainInfo()->m_data.space_rank=1;
				break;
			}
		}
	}
	...
	for(int a=0; a<train.GetTrainRecordCount(); a++)
	{
		...
		if(prec)
		{
			if(TG_IsSpecialGaurdRank(prec->m_data))// bit 1:  重点警卫
			{
				train.GetTrainInfo()->m_data.super_rank=1;
				break;
			}
		}
	}
	...
	for(int a=0; a<train.GetTrainRecordCount(); a++)
	{
		...
		if(prec)
		{
			if(TG_IsMilitaryRank(prec->m_data))// bit 3:  军运
			{
				train.GetTrainInfo()->m_data.military_rank=1;
				break;
			}
		}
	}
