...
public:
	void SetPredictHolder(CTrainDepartPredictHolder *pPredictServer);

private:
	std::map<STATION_NO, int> map_predict_station;

	CTrainDepartPredictHolder *m_pPredictServer;
	void ReportTime(const CTG_SingleTrain& srctrain);
	bool IsPredictStation(STATION_NO station);
private:
	CTgX   *m_pTGCtrl;
	CWnd   *m_pParentWnd;
...
public:
	...
	BOOL   UpdateTrain_Directly(CTG_SingleTrain &train, bool bCheckRelation=false, bool bMoveNextTrain=false);
