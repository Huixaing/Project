	...
	case TG_OPER_UPDATE:
		{
			...
			if(oper_flag.nFlag == CMMIOperFlag::TRAIN_ATTRIB_CHANGED)
			{
			 	if (train.GetSpaceRank()  != prev->GetSpaceRank())
			 	{
					...
					// 仅处理OCX设置超限标志
					train.SetSpaceRank(train.GetTrainInfo()->m_data.space_rank, 0, true, true); // 设置记录的超限标志 
			 	}
			...
			if(GetTGFunctionConfig()->IsEnableNewCzh() && GetTGFunctionConfig()->IsCheckRelationSideCrash())
			{
				CStringArray ary;
			...
			CStringArray alarmAry;
			m_pSchdHolder->CheckTrainManualUpdateOper(&train,m_schd_info,oper_flag,alarmAry);
			if(alarmAry.GetCount()>0)
			{
				CString errinfo;
				GpcLogTrainMessage(0, 0, "修改车次%s(%u)报警:", train.GetTrainID(), train.GetTrainIndex());
				for(int n=0; n<alarmAry.GetCount(); n++)
				{
					errinfo += alarmAry.GetAt(n);
					errinfo += "\n";
					GpcLogTrainMessage(0, 0, "%s", alarmAry.GetAt(n));
				}
