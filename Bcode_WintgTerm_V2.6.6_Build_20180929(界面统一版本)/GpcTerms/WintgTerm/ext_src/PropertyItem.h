...
public:
	CPropertyItemList(LPCTSTR pStrName = NULL);
...
#define DECL_LIST_ITEM_DATA_TYPE(new_class,strlistfunc) \
class new_class : public CPropertyItemList \
{ \
	new_class(const new_class& d); \
	new_class& operator=(const new_class& d); \
public: \
	...
	virtual LPCTSTR GetItemData(int nItem) const; \
private: \
	static LPCTSTR GetItemString(int index); \
}; \
LPCTSTR new_class::GetItemString(int index) \
{  return strlistfunc(index);} \
LPCTSTR new_class::GetItemData(int nItem) const \
{ \
	return  GetItemString(nItem); \
}
