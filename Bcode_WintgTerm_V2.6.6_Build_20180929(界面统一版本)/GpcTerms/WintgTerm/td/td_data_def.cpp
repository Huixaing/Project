...
BOOL TD_ImportRecordInfoFromTD(CTG_TrainRecord &ctcrecord,CTD_TrainRecordInfo &tdrecord,CTD_TrainInfo &info, bool bAcceptSide, CStringArray& list)
{
	...
	if(tdrecord.IMPINFO!=0) 
	{
		...
		if(tdrecord.IMPINFO & 0x01)
		{
			...
			GpcLogTrainMessage(0, 0, "根据TDMS的IMPINFO:%X,设置TID=%s,ST=%s(%d) 为超限",
				tdrecord.IMPINFO, info.TNO, tdrecord.STNM, tdrecord.STID);
		}
		if(tdrecord.IMPINFO & 0x02)
		{
			TG_SetSpecialGaurdRank(ctcrecord.m_data);
			GpcLogTrainMessage(0, 0, "根据TDMS的IMPINFO:%X,设置TID=%s,ST=%s(%d) 为重点警卫",
				tdrecord.IMPINFO, info.TNO, tdrecord.STNM, tdrecord.STID);
		}
		if(tdrecord.IMPINFO & 0x04)
		{
			TG_SetGoustRank(ctcrecord.m_data);
			GpcLogTrainMessage(0, 0, "根据TDMS的IMPINFO:%X,设置TID=%s,ST=%s(%d) 为毒品",
				tdrecord.IMPINFO, info.TNO, tdrecord.STNM, tdrecord.STID);
		}
		if(tdrecord.IMPINFO & 0x08)
		{
			TG_SetMilitaryRank(ctcrecord.m_data);
			GpcLogTrainMessage(0, 0, "根据TDMS的IMPINFO:%X,设置TID=%s,ST=%s(%d) 为军运",
				tdrecord.IMPINFO, info.TNO, tdrecord.STNM, tdrecord.STID);
		}
	}
	...
	{
		GpcLogTrainMessage(0, 0, "根据TDMS的OPERFLAG:%d,设置TID=%s,ST=%s(%d) 禁止通过",
			tdrecord.OPERFLAG, info.TNO,tdrecord.STNM,tdrecord.STID);
	...
	{
		GpcLogTrainMessage(0, 0, "根据TDMS的STPINFO:%d,设置TID=%s,ST=%s(%d) 禁止停车",
			tdrecord.STPINFO, info.TNO, tdrecord.STNM, tdrecord.STID);
		TG_SetFlagMustNotStop(ctcrecord.m_data);
	}
	
	TG_ClearFlagMustStop(ctcrecord.m_data);
	if(tdrecord.STPINFO==2) // 
	{
		GpcLogTrainMessage(0, 0, "根据TDMS的STPINFO:%d,设置TID=%s,ST=%s(%d) 必须停车",
			tdrecord.STPINFO, info.TNO, tdrecord.STNM, tdrecord.STID);
...
static BOOL TD_LoadTrainRecordInfo(MSXML::IXMLDOMNodePtr pNodeRoot,CTD_TrainRecordInfo &recordinfo)
{
		...
		GET_NODE_UINTVALUE(recordinfo.IMPINFO,pNodeRoot,"IMPINFO")
	...
	for(int idx = 0; idx < rec_count; idx++)
	{
		...
		if(idx<reclist.GetRecordCount())
		{
			if(reclist[idx].GetStationNo()== recordinfo.STID)
			{
				...
				// 20161123 hjz 接收调整计划时 如果新线有延长 将旧线的终到点改为自触
				if (rec_count>=reclist.GetRecordCount() && reclist[idx].IsTerminalRec() && (idx+1==reclist.GetRecordCount())) 
				{
					if(newrecord.IsActualDepartTimeRecord())
					{
						GpcLogTrainMessage(0, 0, "TID=%s,ST=%s(%d) 为实际点,保留自动触发方式",
									traininfo.TNO,recordinfo.STNM,recordinfo.STID);
					}
					else
					{
						GpcLogTrainMessage(0, 0, "TID=%s,ST=%s(%d) 为计划点,设置发车进路自动触发",
									traininfo.TNO,recordinfo.STNM,recordinfo.STID);
