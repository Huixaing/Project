...
BOOL  cStPlan::InitStationPlanReceiveInfo(int section_id)
{
	...
	for(int k=0;k < nEntityNum;k++)
	{
		...
		int station=rec.nstationNo;
		if(std::find(m_send_plan_station.begin(), m_send_plan_station.end(), station)
			== m_send_plan_station.end())
		{
			m_send_plan_station.push_back(station);
		}
...
void cStPlan::GetSendPlanStation(std::vector<int>& station_list) const
{
	station_list.clear();
	station_list.assign(m_send_plan_station.begin(), m_send_plan_station.end());
}
